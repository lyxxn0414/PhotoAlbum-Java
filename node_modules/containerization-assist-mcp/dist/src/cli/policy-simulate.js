...
import { loadRegoPolicy } from '../config/policy-rego.js';
...
async function simulatePolicy(options) {
    ...
    const outputWithout = await toolHandler.handler(input, contextWithout);
    // Query policy data (without applying it)
    const configWithout = null;
    const knowledgeWithout = [];
    const templatesWithout = [];
    const validationWithout = [];
    // Run WITH policy
    ...
    const contextWith = {
        ...
        // signal and progress are omitted (optional)
        policy,
        queryConfig: async (packageName, policyInput) => {
            return policy.queryConfig(packageName, policyInput);
        },
    };
    const outputWith = await toolHandler.handler(input, contextWith);
    // Query policy data
...
function printSimulationResults(result) {
    console.log(`\n${'='.repeat(80)}`);
    ...
    if (differences.knowledgeFiltered > 0) {
        ...
        withPolicy.knowledgeFiltering.forEach((filter, i) => {
            console.log(`  ${i + 1}. Action: ${filter.action}`);
            if (filter.pattern)
                console.log(`     Pattern: ${filter.pattern}`);
            if (filter.tags)
                console.log(`     Tags: ${filter.tags.join(', ')}`);
            if (filter.reason)
    ...
    if (differences.templatesInjected > 0) {
        console.log(`📝 Templates Injected (${differences.templatesInjected} templates):`);
        withPolicy.templates.forEach((template, i) => {
    ...
    if (differences.validationRulesAdded > 0) {
        ...
        withPolicy.validationRules.forEach((rule, i) => {
    ...
    if (differences.outputDiffers) {
        ...
        if (withoutPolicy.output && typeof withoutPolicy.output === 'object' && 'summary' in withoutPolicy.output) {
            console.log(`  Summary: ${withoutPolicy.output.summary}`);
            if ('recommendations' in withoutPolicy.output) {
                const recs = withoutPolicy.output.recommendations;
        ...
        if (withPolicy.output && typeof withPolicy.output === 'object' && 'summary' in withPolicy.output) {
            console.log(`  Summary: ${withPolicy.output.summary}`);
            if ('recommendations' in withPolicy.output) {
                const recs = withPolicy.output.recommendations;
                const totalRecs = (recs.securityConsiderations?.length || 0) + (recs.bestPractices?.length || 0);
                console.log(`  Recommendations: ${totalRecs} total`);
