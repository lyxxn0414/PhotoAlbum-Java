...
import { config, logConfigSummaryIfDev } from '../config/index.js';
import { createLogger } from '../lib/logger.js';
import { exit, argv, env, cwd } from 'node:process';
import { readFileSync } from 'node:fs';
import { join, dirname, basename } from 'node:path';
import { fileURLToPath } from 'node:url';
import { checkDockerHealth, checkKubernetesHealth } from '../infra/health/checks.js';
import { validateDockerSocket } from '../infra/docker/socket-validation.js';
import { createInspectToolsCommand } from './commands/inspect-tools.js';
import { provideContextualGuidance } from './guidance.js';
import { validateOptions } from './validation.js';
import { OUTPUTFORMAT } from '../mcp/mcp-server.js';
import { logStartup, logStartupSuccess, installShutdownHandlers, logStartupFailure, } from '../lib/runtime-logging.js';
import { discoverPolicies } from '../app/orchestrator.js';
 ...
 */
async function handleListPoliciesCommand(opts) {
...
async function main() {
    try {
        ...
        if (options.workspace)
            env.WORKSPACE_DIR = options.workspace;
        ...
        if (options.validate) {
            ...
            console.error(dockerStatus.available
                ? `  ✅ Docker: ${dockerStatus.version}`
                : `  ⚠️  Docker: ${dockerStatus.error}`);
            console.error(k8sStatus.available
        ...
        }, getLogger(), !!process.env.MCP_QUIET);
        ...
        logStartupFailure(error, getLogger(), !!process.env.MCP_QUIET);
