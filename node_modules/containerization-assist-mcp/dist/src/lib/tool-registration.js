...
import { formatOutput, OUTPUTFORMAT } from '../mcp/mcp-server.js';
 ...
 * ```
 */
export function createToolHandler(app, toolName, options = {}) {
    const { transport = 'external', outputFormat = app.config?.outputFormat ?? OUTPUTFORMAT.NATURAL_LANGUAGE, chainHintsMode = app.config?.chainHintsMode ?? CHAINHINTSMODE.ENABLED, onError, onSuccess, } = options;
 ...
 * ```
 */
export function registerTool(server, app, tool, options) {
 ...
 * ```
 */
export function registerTools(server, app, tools, options) {
