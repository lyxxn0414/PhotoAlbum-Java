 ...
 */
function filterDockerfileTemplates(templates, context) {
 ...
 */
function filterK8sTemplates(templates, context) {
    return templates.filter((template) => {
        if (!template.conditions) {
            return true;
        }
 ...
 */
export function dockerfileTemplatesToRecommendations(templates, context) {
 ...
 */
export function k8sTemplatesToRecommendations(templates, context) {
    // Filter templates by conditions
 ...
 */
export function mergeTemplateRecommendations(existing, templates) {
 ...
 */
export function mergeTemplatesIntoPlan(plan, templateAdditions, context) {
    ...
    if (templateAdditions.dockerfile && templateAdditions.dockerfile.length > 0) {
        const dockerfileTemplateRecs = dockerfileTemplatesToRecommendations(templateAdditions.dockerfile, context);
        ...
        if ('securityConsiderations' in updatedPlan.recommendations) {
            const securityTemplates = dockerfileTemplateRecs.filter((rec) => rec.category.includes('security'));
            ...
            if (Array.isArray(securityRecs)) {
                updatedPlan.recommendations = {
                    ...updatedPlan.recommendations,
                    securityConsiderations: mergeTemplateRecommendations(securityRecs, securityTemplates),
                };
            }
        }
        if ('bestPractices' in updatedPlan.recommendations) {
            const otherTemplates = dockerfileTemplateRecs.filter((rec) => !rec.category.includes('security'));
            const bestPracticesRecs = updatedPlan.recommendations.bestPractices;
            if (Array.isArray(bestPracticesRecs)) {
                updatedPlan.recommendations = {
                    ...updatedPlan.recommendations,
                    bestPractices: mergeTemplateRecommendations(bestPracticesRecs, otherTemplates),
                };
            }
        }
    }
    ...
    if (templateAdditions.kubernetes && templateAdditions.kubernetes.length > 0) {
        ...
        if ('securityConsiderations' in updatedPlan.recommendations) {
            const securityTemplates = k8sTemplateRecs.filter((rec) => rec.category.includes('security') || rec.recommendation.toLowerCase().includes('security'));
            const securityRecs = updatedPlan.recommendations.securityConsiderations;
            if (Array.isArray(securityRecs)) {
                updatedPlan.recommendations = {
                    ...updatedPlan.recommendations,
        ...
        if ('bestPractices' in updatedPlan.recommendations) {
            const otherTemplates = k8sTemplateRecs.filter((rec) => !rec.category.includes('security') && !rec.recommendation.toLowerCase().includes('security'));
            const bestPracticesRecs = updatedPlan.recommendations.bestPractices;
            if (Array.isArray(bestPracticesRecs)) {
                updatedPlan.recommendations = {
                    ...updatedPlan.recommendations,
 ...
 */
export function extractTemplateAdditions(generationConfig) {
