 ...
 */
function tryResolveCJSPath(relativePathFromDistSrc, logger) {
    try {
 ...
 */
function tryResolveESMPath(moduleUrl, relativePathFromDistSrc, logger) {
    if (!moduleUrl) {
        return { resolved: false };
    }
    try {
 ...
 */
function resolveSymlink(path, logger) {
    try {
 ...
 */
function tryResolveFromArgv(targetSubdir, logger) {
    if (!process.argv[1]) {
        return { resolved: false };
    }
    try {
        ...
        logger.debug({ error, argv1: process.argv[1] }, 'Failed to resolve module path from process.argv[1]');
    }
    return { resolved: false };
}
/**
 ...
 */
function generateCwdSearchPaths(targetSubdir) {
    const paths = [];
 ...
 */
export function resolveModulePaths(options) {
 ...
 */
export function findExistingDirectory(searchPaths, logger) {
...
//# sourceMappingURL=module-path-resolver.js.map