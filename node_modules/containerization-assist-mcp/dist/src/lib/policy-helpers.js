 ...
 */
import { applyPolicy } from '../config/policy-eval.js';
/**
 ...
 */
export function mapRegoPolicyViolation(regoViolation) {
    const violation = {
 ...
 * ```
 */
export async function validateContentAgainstPolicy(content, policyEvaluator, logger, contentType = 'content') {
    ...
    const violations = policyResult.violations.map(mapRegoPolicyViolation);
    const warnings = policyResult.warnings.map(mapRegoPolicyViolation);
    const suggestions = policyResult.suggestions.map(mapRegoPolicyViolation);
    const passed = violations.length === 0;
    const totalRules = (policyResult.summary?.total_violations || 0) +
...
//# sourceMappingURL=policy-helpers.js.map