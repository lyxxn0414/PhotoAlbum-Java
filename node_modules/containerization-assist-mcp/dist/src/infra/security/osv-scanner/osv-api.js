...
import { parseCVSSVector } from './cvss-parser.js';
class RateLimiter {
    tokens;
    lastRefill;
    maxTokens;
...
async function fetchVulnerabilityDetails(vulnId, _logger) {
...
async function fetchBatchVulnerabilityIds(packages, logger) {
    const vulnIdsByPackage = new Map();
                    ...
                    for (let j = 0; j < data.results.length; j++) {
                        ...
                        if (result?.vulns?.length) {
                            ...
                            if (pkg) {
                                ...
                                vulnIdsByPackage.set(pkgKey, result.vulns.map((v) => v.id));
                            }
                        }
                    }
                    break;
...
export async function queryOSVBatch(packages, logger) {
    ...
    const vulnDetailsCache = new Map();
