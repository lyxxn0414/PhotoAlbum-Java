...
export const ScannerErrors = {
     ...
     */
    invalidImageId: (imageId) => Failure('Invalid imageId format', {
     ...
     */
    scannerNotInstalled: (scannerName, installUrl) => Failure(`${scannerName} not installed or not in PATH`, {
     ...
     */
    jsonParseError: (scannerName, parseError, outputPreview) => Failure(`Failed to parse ${scannerName} output`, {
     ...
     */
    emptyOutput: (scannerName, imageId) => Failure(`${scannerName} returned empty output`, {
     ...
     */
    scanExecutionError: (scannerName, imageId, errorMessage) => Failure(`${scannerName} scan failed: ${errorMessage}`, {
     ...
     */
    versionCheckTimeout: (scannerName, command) => Failure(`${scannerName} version check timed out`, {
     ...
     */
    versionParseError: (scannerName, command) => Failure(`${scannerName} version could not be parsed`, {
 ...
 */
export function parseVersion(output, pattern) {
 ...
 */
export function logScanStart(logger, scanner, version, imageId) {
    logger.info({ scanner, version, imageId }, `Starting ${scanner} scan`);
}
/**
 * Log scan completion
 */
export function logScanComplete(logger, scanner, imageId, totalVulnerabilities, criticalCount, highCount) {
    logger.info({
        scanner,
        imageId,
...
//# sourceMappingURL=scanner-common.js.map