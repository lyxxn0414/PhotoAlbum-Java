...
import { Failure, TOPICS } from '../../types/index.js';
import { generateDockerfileSchema, } from './schema.js';
import { CATEGORY } from '../../knowledge/types.js';
import { createKnowledgeTool, createSimpleCategorizer } from '../shared/knowledge-tool-pattern.js';
import { promises as fs } from 'node:fs';
import nodePath from 'node:path';
import { validateContentAgainstPolicy, } from '../../lib/policy-helpers.js';
const name = 'generate-dockerfile';
const description = 'Gather insights from knowledgebase and return requirements for Dockerfile creation or enhancement. Automatically detects existing Dockerfiles and provides detailed analysis and guidance.';
const version = '2.0.0';
/**
...
async function isImageCompliant(image, evaluator, logger) {
    ...
    const validation = await validateContentAgainstPolicy(dockerfileText, evaluator, logger, 'base image');
...
async function handleGenerateDockerfile(input, ctx) {
    ...
    if (ctx.policy) {
        ...
        if (templateQuery) {
            ...
            const { mergeTemplatesIntoPlan } = await import('../../lib/template-merger.js');
...
export default tool({
    ...
    chainHints: {
        success: 'Dockerfile plan generated successfully and passed policy validation. Next: Use fix-dockerfile to validate the actual Dockerfile content before building.',
        failure: 'Failed to generate Dockerfile plan or plan violates policies. Review repository analysis and policy violations.',
    },
