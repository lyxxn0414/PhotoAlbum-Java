...
import { getToolLogger } from '../../lib/tool-helpers.js';
import { validateContentAgainstPolicy, } from '../../lib/policy-helpers.js';
import { generateK8sManifestsToolDefinition } from './types.js';
...
async function handleGenerateK8sManifests(input, ctx) {
    ...
    if (input.acaManifest) {
        ...
        if (input.manifestType && input.manifestType !== 'kubernetes') {
            return Failure(`manifestType '${input.manifestType}' is not supported in ACA conversion mode. ACA conversion always produces Kubernetes manifests.`, {
    ...
    if (ctx.policy) {
        ...
        if (!policyValidation.passed) {
            const violationMessages = policyValidation.violations
                ...
                .join('\n');
            return Failure(`Generated manifest plan violates organizational policies:\n${violationMessages}`, {
