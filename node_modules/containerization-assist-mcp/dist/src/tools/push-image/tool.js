 ...
 */
import { createDockerClient } from '../../infra/docker/client.js';
import { getRegistryCredentials } from '../../infra/docker/credential-helpers.js';
import { getToolLogger } from '../../lib/tool-helpers.js';
import { parseImageName } from '../../lib/validation-helpers.js';
import { Success, Failure } from '../../types/index.js';
import { tool } from '../../types/tool.js';
import { pushImageSchema } from './schema.js';
import { createErrorGuidance } from '../../lib/errors.js';
/**
 * Push image handler
 */
async function handlePushImage(input, ctx) {
    ...
    try {
        ...
        if (!input.imageId) {
            return Failure('Missing required parameter: imageId', createErrorGuidance('Missing required parameter: imageId', 'The imageId parameter is required to push an image', 'Provide the imageId of the Docker image to push. Use `docker images` to list available images.'));
        }
        ...
        if (!authConfig && input.credentials) {
            ...
            if (!input.credentials.username || !input.credentials.password) {
                return Failure('Missing registry credentials', createErrorGuidance('Both username and password are required for registry authentication', 'Registry credentials are incomplete', 'Provide both username and password in the credentials parameter'));
            }
        ...
        if (!tagResult.ok) {
            return Failure(`Failed to tag image: ${tagResult.error}`, tagResult.guidance ||
                createErrorGuidance(tagResult.error, 'Unable to tag the Docker image', 'Verify the image exists with `docker images` and the tag format is valid.'));
        }
...
export default tool({
    ...
    chainHints: {
        ...
        failure: 'Image push failed. Check registry credentials, network connectivity, and image tag format.',
    },
