...
import { validateNamespace } from '../../lib/validation.js';
import { DEFAULT_TIMEOUTS, DOCKER, KUBERNETES } from '../../config/constants.js';
import { createKubernetesClient, } from '../../infra/kubernetes/client.js';
import { getSystemInfo, getDownloadOS, getDownloadArch, mapNodeArchToPlatform, isPlatformCompatible, } from '../../lib/platform.js';
import { downloadFile, makeExecutable, createTempFile, deleteTempFile } from '../../lib/file-utils.js';
import { findRegistryPort } from '../../lib/port-utils.js';
import { Success, Failure } from '../../types/index.js';
import { prepareClusterSchema } from './schema.js';
import { exec } from 'node:child_process';
import { promisify } from 'node:util';
import { pluralize } from '../../lib/summary-helpers.js';
...
function validateAndEscapeClusterName(clusterName) {
    ...
    if (!nameRegex.test(clusterName)) {
        return Failure(`Invalid cluster name: "${clusterName}". Must contain only lowercase letters, numbers, and hyphens.`, {
...
async function validateExistingClusterArchitecture(clusterName, targetPlatform, strictMode, logger) {
    ...
    if (clusterPlatform !== targetPlatform) {
        return Failure(`Existing cluster '${clusterName}' is ${clusterPlatform} but target platform is ${targetPlatform}`, {
...
async function checkLocalRegistryExists(logger) {
    try {
        ...
        const { stdout: allContainers } = await execAsync(`docker ps -a --filter "name=${DOCKER.REGISTRY_CONTAINER_NAME}" --format "{{.Names}}"`);
        ...
        const { stdout: portMapping } = await execAsync(`docker inspect ${DOCKER.REGISTRY_CONTAINER_NAME} --format '{{range $p, $conf := .NetworkSettings.Ports}}{{if eq $p "5000/tcp"}}{{(index $conf 0).HostPort}}{{end}}{{end}}'`);
        ...
        const { stdout: runningContainers } = await execAsync(`docker ps --filter "name=${DOCKER.REGISTRY_CONTAINER_NAME}" --format "{{.Names}}"`);
...
async function validateRegistryHealth(port, logger) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            const { stdout: status } = await execAsync(`docker inspect ${DOCKER.REGISTRY_CONTAINER_NAME} --format '{{.State.Status}}'`);
        ...
        try {
            const { stdout } = await execAsync(`curl -sf --max-time 3 http://${DOCKER.REGISTRY_HOST}:${port}/v2/ || echo "failed"`, { timeout: 4000 });
    ...
    try {
        const { stdout: logs } = await execAsync(`docker logs --tail 20 ${DOCKER.REGISTRY_CONTAINER_NAME}`);
...
async function validateContainerdConfig(clusterName, port, logger) {
    try {
        ...
        const mirrorHostPattern = new RegExp(`\\[plugins\\."io\\.containerd\\.grpc\\.v1\\.cri"\\.registry\\.mirrors\\."${DOCKER.REGISTRY_HOST}:${port}"\\]`);
        ...
        const endpointPattern = new RegExp(`endpoint\\s*=\\s*\\["http://${DOCKER.REGISTRY_CONTAINER_NAME}:${DOCKER.REGISTRY_INTERNAL_PORT}"\\]`);
...
async function createRegistryContainerOnly(port, logger) {
    try {
        ...
        await execAsync(`docker run -d --restart=always -p ${port}:${DOCKER.REGISTRY_INTERNAL_PORT} --name ${DOCKER.REGISTRY_CONTAINER_NAME} registry:2`);
