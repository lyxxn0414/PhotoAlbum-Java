...
import { createSecurityScanner } from '../../infra/security/scanner.js';
import { Success, Failure } from '../../types/index.js';
import { getKnowledgeForCategory } from '../../knowledge/index.js';
import { formatVulnerabilities, buildStatusSummary, pluralize } from '../../lib/summary-helpers.js';
import { scanImageToolDefinition } from './types.js';
/**
...
async function handleScanImage(params, context) {
    ...
    try {
        ...
        if (!scanResultWrapper.ok) {
            return Failure(`Failed to scan image: ${scanResultWrapper.error ?? 'Unknown error'}`, scanResultWrapper.guidance);
        }
        ...
        const remediationText = remediationGuidance.length > 0
            ...
            : '';
        const summary = buildStatusSummary(passed, `🔒 Security scan passed (${scanner}). ${vulnSummary}.${remediationText}`, `🔒 Security scan failed (${scanner}). ${vulnSummary}.${remediationText}`);
