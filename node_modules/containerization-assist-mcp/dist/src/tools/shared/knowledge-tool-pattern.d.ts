...
import type { KnowledgeSnippet } from '../../knowledge/schemas.js';
/**
...
export interface RulesConfig<TInput, TRuleResults> {
     ...
     */
    applyRules: (input: TInput, knowledge: CategorizedKnowledge, ctx: ToolContext) => TRuleResults | Promise<TRuleResults>;
}
/**
...
export interface PlanBuilderConfig<TInput, TRuleResults, TPlan> {
     ...
     */
    buildPlan: (input: TInput, knowledge: CategorizedKnowledge, rules: TRuleResults, confidence: number, ctx: ToolContext) => TPlan | Promise<TPlan>;
}
/**
 * Complete configuration for a knowledge-based tool.
 * Combines all aspects of the pattern into a single config object.
 */
export interface KnowledgeToolConfig<TInput, TPlan, TCategories extends string = string, TRuleResults = Record<string, unknown>> {
 ...
 */
export declare const defaultConfidenceCalculation: (matchCount: number) => number;
/**
 ...
 * ```
 */
export declare function createKnowledgeTool<TInput, TPlan, TCategories extends string = string, TRuleResults = Record<string, unknown>>(config: KnowledgeToolConfig<TInput, TPlan, TCategories, TRuleResults>): (input: TInput, ctx: ToolContext) => Promise<Result<TPlan>>;
/**
 ...
 * ```
 */
export declare function createSimpleCategorizer<TCategories extends string>(rules: Record<TCategories, (snippet: KnowledgeSnippet) => boolean>): (snippet: KnowledgeSnippet) => TCategories[];
//# sourceMappingURL=knowledge-tool-pattern.d.ts.map