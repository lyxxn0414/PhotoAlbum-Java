...
export function mergeReports(a, b) {
    ...
    for (const result of b.results) {
        ...
        if (byKey.has(key)) {
            ...
            if (existing) {
                const existingSeverity = existing.metadata?.severity || 'info';
                const newSeverity = result.metadata?.severity || 'info';
 ...
 */
export function mergeMultipleReports(reports) {
    if (reports.length === 0) {
        return {
            results: [],
            score: 100,
