...
import { ValidationSeverity, ValidationCategory, } from './core-types.js';
import { lintWithDockerfilelint } from './dockerfilelint-adapter.js';
import { mergeReports } from './merge-reports.js';
import { SUDO_INSTALL, AS_CLAUSE, LATEST_TAG, PACKAGE_FILES, PASSWORD_PATTERN, API_KEY_PATTERN, SECRET_PATTERN, TOKEN_PATTERN, } from '../lib/regex-patterns.js';
/**
 * Get argument value from docker command
 */
const getArgValue = (command) => {
...
const DOCKERFILE_RULES = [
    {
        ...
        check: (commands) => {
            const userCommands = commands.filter((cmd) => cmd.name === 'USER');
    ...
    },
    {
        ...
        check: (commands) => {
            const runCommands = commands.filter((cmd) => cmd.name === 'RUN');
            return !runCommands.some((cmd) => {
    ...
    },
    {
        ...
        check: (commands) => {
            return commands.some((cmd) => cmd.name === 'HEALTHCHECK');
        },
    ...
    },
    {
        ...
        check: (commands) => {
            const fromCount = commands.filter((cmd) => cmd.name === 'FROM').length;
    ...
    },
    {
        ...
        check: (commands) => {
            const hasCmd = commands.some((cmd) => cmd.name === 'CMD' || cmd.name === 'ENTRYPOINT');
            const hasExpose = commands.some((cmd) => cmd.name === 'EXPOSE');
            if (!hasCmd)
                return true;
            return hasExpose;
        },
    ...
    },
    {
        ...
        check: (commands) => {
            return commands.some((cmd) => cmd.name === 'WORKDIR');
        },
 ...
 */
const isHealthCheckLine = (lines, lineNumber) => {
 ...
 */
const validateSyntax = (content) => {
    const basicValidation = validateDockerfileSyntax(content);
    if (!basicValidation.valid) {
        ...
        const syntaxErrors = basicValidation.errors?.filter((err) => err.priority === 0 &&
 ...
 */
const calculateGrade = (score, hasCriticalSecurity) => {
    let grade;
    if (score >= 90)
        grade = 'A';
    else if (score >= 80)
...
const createReport = (results) => {
    const errors = results.filter((r) => !r.passed && r.metadata?.severity === ValidationSeverity.ERROR).length;
    const warnings = results.filter((r) => !r.passed && r.metadata?.severity === ValidationSeverity.WARNING).length;
    const info = results.filter((r) => !r.passed && r.metadata?.severity === ValidationSeverity.INFO).length;
    const passed = results.filter((r) => r.passed).length;
    const total = results.length;
    // Calculate weighted score
    const deductions = errors * SEVERITY_WEIGHTS[ValidationSeverity.ERROR] +
    ...
    const hasCriticalSecurity = results.some((r) => SECURITY_RULES.includes(r.ruleId || '') &&
 ...
 */
function detectBuildKitFeatures(content) {
    const lines = content.split('\n');
 ...
 */
function validateBuildKitDockerfile(content) {
    const results = [];
    const lines = content.split('\n');
 ...
 */
export const validateDockerfileContent = async (dockerfileContent, options) => {
    ...
    if (buildKit.syntax || buildKit.hasHeredocs || buildKit.hasMounts) {
        ...
        if (parseResult.ok) {
            ...
            for (const rule of validationRules) {
                ...
                if (!passed && rule.id === 'no-secrets') {
                    ...
                    const secretVariable = commands.find((cmd) => {
                        if (cmd.name === 'ENV' || cmd.name === 'ARG') {
                            const value = getArgValue(cmd);
                            const suspicious = [PASSWORD_PATTERN, API_KEY_PATTERN, SECRET_PATTERN, TOKEN_PATTERN];
                            return suspicious.some((pattern) => pattern.test(value));
        ...
        if (options?.enableExternalLinter !== false) {
            try {
                ...
                console.warn('External linter failed with BuildKit file, using BuildKit-specific validation:', error);
