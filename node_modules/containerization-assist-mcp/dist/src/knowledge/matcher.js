...
import { loadKnowledgeData } from './loader.js';
 ...
 */
const evaluatePatternMatch = (entry, query) => {
    if (!query.text || !entry.pattern)
    ...
    try {
        ...
        logger.debug({ entryId: entry.id, pattern: entry.pattern, error }, 'Skipping entry with invalid pattern');
    }
    return { score: 0, reasons: [] };
};
/**
 ...
 */
const evaluateTagMatch = (entry, query) => {
 ...
 */
const evaluateLanguageMatch = (entry, query) => {
    if (!query.language)
        return { score: 0, reasons: [] };
    ...
    if (score === 0) {
        const textMatch = languageKeywords.some((keyword) => {
            ...
            return (entry.recommendation.toLowerCase().includes(lowerKeyword) ||
 ...
 */
const evaluateFrameworkMatch = (entry, query) => {
    if (!query.framework || !entry.tags)
        return { score: 0, reasons: [] };
    const frameworkKeywords = getFrameworkKeywords(query.framework);
    const hasMatch = frameworkKeywords.some((keyword) => entry.tags?.includes(keyword) || entry.recommendation.toLowerCase().includes(keyword));
    if (hasMatch) {
        return {
 ...
 */
const evaluateEnvironmentMatch = (entry, query) => {
    if (!query.environment)
        return { score: 0, reasons: [] };
    const envKeywords = getEnvironmentKeywords(query.environment);
    const hasMatch = envKeywords.some((keyword) => entry.tags?.includes(keyword) || entry.recommendation.toLowerCase().includes(keyword));
    if (hasMatch) {
        return {
 ...
 */
const evaluateToolMatch = (entry, query) => {
    if (!query.tool || !entry.tags)
        return { score: 0, reasons: [] };
 ...
 */
const hasConflictingLanguageTag = (entry, queryLanguage, queryTags) => {
    ...
    const entryContextTags = entry.tags
        .map(normalizeTag)
        .filter((tag) => LANGUAGE_TAGS.includes(tag));
    ...
    const hasMatchingContext = entryContextTags.some((tag) => tag === normalizedQueryLanguage ||
 ...
 */
const evaluateEntry = (entry, query) => {
 ...
 */
export const findKnowledgeMatches = (entries, query) => {
 ...
 */
export async function getKnowledgeSnippets(topic, options) {
    try {
        const knowledgeData = await loadKnowledgeData();
        const queryTextParts = [topic];
        if (options.detectedDependencies && options.detectedDependencies.length > 0) {
            queryTextParts.push(...options.detectedDependencies);
        }
        const queryTags = [options.tool];
        if (options.language)
            queryTags.push(options.language);
        if (options.framework)
            queryTags.push(options.framework);
        if (options.environment)
            queryTags.push(options.environment);
 ...
 */
function extractFromLines(example) {
 ...
 */
function formatEntryAsSnippet(entry) {
    const parts = [];
 ...
 */
function applyCharacterBudget(snippets, maxChars) {
    const selected = [];
