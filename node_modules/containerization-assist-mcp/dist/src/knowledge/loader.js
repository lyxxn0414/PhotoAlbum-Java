...
import { KnowledgeEntrySchema, KnowledgePackSchema } from './schemas.js';
import { z } from 'zod';
// ===== Constants =====
 ...
 */
export function discoverBuiltInKnowledgePacks() {
    try {
 ...
 */
function validateAndNormalizePack(packFile, data) {
    try {
        const validated = KnowledgePackSchema.parse(data);
        // Extract entries based on format
        const entries = Array.isArray(validated)
            ? validated // Format 1: Flat array
            : validated.rules; // Format 2: Object with rules
 ...
 */
function loadPackFile(packPath, stats) {
    try {
...
export function loadKnowledgeBase() {
    ...
    if (packPaths.length === 0) {
        ...
        const error = new Error(`No knowledge packs discovered - server cannot start without knowledge base.\n` +
            `\n` +
