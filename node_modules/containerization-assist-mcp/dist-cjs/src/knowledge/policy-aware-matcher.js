...
exports.findPolicyAwareKnowledgeMatches = findPolicyAwareKnowledgeMatches;
exports.getPolicyAwareKnowledgeSnippets = getPolicyAwareKnowledgeSnippets;
const logger_1 = require("../lib/logger");
const matcher_1 = require("./matcher");
const logger = (0, logger_1.createLogger)().child({ module: 'policy-aware-matcher' });
/**
 ...
 */
async function queryKnowledgeFilter(policy, context) {
    try {
        logger.debug({ context }, 'Querying policy for knowledge filters');
        const startTime = Date.now();
        // Query the policy for knowledge filter configuration
        const filter = await policy.queryConfig(KNOWLEDGE_FILTER_PACKAGE, context);
 ...
 */
function applyExclusionFilters(entries, filter) {
    const excluded = [];
 ...
 */
function applyWeightMultipliers(matches, filter) {
    const boosted = [];
    const reduced = [];
 ...
 */
function applyMinScore(matches, minScore) {
 ...
 */
async function findPolicyAwareKnowledgeMatches(entries, query, policy) {
    // If no policy, use standard matcher
    if (!policy) {
        const matches = (0, matcher_1.findKnowledgeMatches)(entries, query);
        return {
            matches,
            filterResult: {
                excluded: [],
    ...
    let matches = (0, matcher_1.findKnowledgeMatches)(filteredEntries, query);
 ...
 */
async function getPolicyAwareKnowledgeSnippets(entries, query, policy) {
...
//# sourceMappingURL=policy-aware-matcher.js.map