...
exports.formatScanImageNarrative = formatScanImageNarrative;
exports.formatDockerfilePlanNarrative = formatDockerfilePlanNarrative;
exports.formatBuildImageNarrative = formatBuildImageNarrative;
exports.formatAnalyzeRepoNarrative = formatAnalyzeRepoNarrative;
exports.formatVerifyDeployNarrative = formatVerifyDeployNarrative;
exports.formatFixDockerfileNarrative = formatFixDockerfileNarrative;
exports.formatGenerateK8sManifestsNarrative = formatGenerateK8sManifestsNarrative;
exports.formatPushImageNarrative = formatPushImageNarrative;
exports.formatTagImageNarrative = formatTagImageNarrative;
exports.formatPrepareClusterNarrative = formatPrepareClusterNarrative;
exports.formatOpsPingNarrative = formatOpsPingNarrative;
exports.formatOpsStatusNarrative = formatOpsStatusNarrative;
 ...
 */
function formatScanImageNarrative(result, chainHintsMode = orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
    const parts = [];
    // Header
    ...
    const vulnText = (0, summary_helpers_1.formatVulnerabilities)({
    ...
    if (result.recommendedActions && result.recommendedActions.length > 0) {
        const totalFixed = result.recommendedActions.reduce((sum, action) => sum + action.vulnerabilitiesFixed, 0);
        const actionLabel = (0, summary_helpers_1.pluralize)(result.recommendedActions.length, 'action');
        const vulnerabilityLabel = (0, summary_helpers_1.pluralize)(totalFixed, 'vulnerability', 'vulnerabilities');
    ...
    if (result.vulnerabilityDetails && result.vulnerabilityDetails.length > 0) {
        ...
        if (fixableVulns.length > 0) {
            ...
            sortedFixable.forEach((vuln, idx) => {
                parts.push(`  ${idx + 1}. [${vuln.severity}] ${vuln.package}: ${vuln.version} → ${vuln.fixedVersion}`);
    ...
    if (chainHintsMode === orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
        parts.push('\n**Next Steps:**');
        if (result.passed) {
 ...
 */
function formatDockerfilePlanNarrative(plan, chainHintsMode = orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
    const parts = [];
 ...
 */
function formatBuildImageNarrative(result, chainHintsMode = orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
    const parts = [];
    // Header
 ...
 */
function formatAnalyzeRepoNarrative(result, chainHintsMode = orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
    const parts = [];
    // Header
 ...
 */
function formatVerifyDeployNarrative(result, chainHintsMode = orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
    const parts = [];
    // Header
    ...
    if (result.pods && result.pods.length > 0) {
        ...
        parts.push(`\n**Pod Status:**`);
        if (runningPods > 0)
            parts.push(`  ✅ Running: ${runningPods}`);
        if (pendingPods > 0)
            parts.push(`  ⏳ Pending: ${pendingPods}`);
        if (failedPods > 0)
            parts.push(`  ❌ Failed: ${failedPods}`);
 ...
 */
function formatFixDockerfileNarrative(result, chainHintsMode = orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
    const parts = [];
    ...
    parts.push(`${gradeIcon[result.validationGrade] || '❓'} Dockerfile Validation: Grade ${result.validationGrade} (Score: ${result.validationScore}/100)\n`);
    // Priority indicator
    const priorityIcon = {
        high: '🔴',
        medium: '🟡',
    ...
    const totalIssues = result.currentIssues.security.length +
    ...
    // Fix recommendations
    const totalFixes = result.fixes.security.length +
    ...
    if (totalFixes > 0) {
        ...
        if (result.fixes.security.length > 0) {
            ...
            result.fixes.security.slice(0, 3).forEach((fix, idx) => {
                ...
                if (fix.example) {
                    const truncatedExample = fix.example.length > 80 ? `${fix.example.substring(0, 77)}...` : fix.example;
 ...
 */
function formatGenerateK8sManifestsNarrative(result, chainHintsMode = orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
    const parts = [];
    // Action-oriented header
    ...
    if (result.acaAnalysis) {
        ...
        result.acaAnalysis.containerApps.forEach((app, idx) => {
            ...
            if (app.hasIngress)
                parts.push(`     ✓ Ingress configured`);
            if (app.hasScaling)
                parts.push(`     ✓ Scaling enabled`);
            if (app.hasSecrets)
                parts.push(`     ✓ Secrets configured`);
        });
 ...
 */
function formatPushImageNarrative(result, chainHintsMode = orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
    const parts = [];
    // Header
 ...
 */
function formatTagImageNarrative(result, chainHintsMode = orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
    const parts = [];
    // Header
 ...
 * - Context-aware next steps (when chainHintsMode is 'enabled')
 */
function formatPrepareClusterNarrative(result, chainHintsMode = orchestrator_types_1.CHAINHINTSMODE.ENABLED) {
    const parts = [];
    // Header
 ...
 */
function formatOpsPingNarrative(result) {
    const parts = [];
    // Header
    ...
    parts.push(`  Uptime: ${(0, summary_helpers_1.formatDuration)(Math.round(result.server.uptime))}`);
    parts.push(`  Process ID: ${result.server.pid}`);
    // Capabilities
    parts.push(`\n**Capabilities:**`);
    if (result.capabilities.tools)
        parts.push('  ✅ Tools available');
    if (result.capabilities.progress)
        parts.push('  ✅ Progress tracking enabled');
    // Status
 ...
 */
function formatOpsStatusNarrative(result) {
    const parts = [];
    // Header
    const icon = result.success ? '💚' : '❌';
    ...
    parts.push(`**Uptime:** ${(0, summary_helpers_1.formatDuration)(Math.round(result.uptime))}`);
    // Memory usage
    if (result.memory) {
...
//# sourceMappingURL=natural-language-formatters.js.map