 ...
 */
async function checkDeploymentHealth(k8sClient, namespace, deploymentName, timeout) {
...
async function checkEndpointHealth(url) {
    try {
        ...
        const timeoutId = setTimeout(() => controller.abort(), constants_1.DEFAULT_TIMEOUTS.healthCheck || 5000);
        try {
 ...
 */
async function handleVerifyDeployment(params, context) {
    if (!params || typeof params !== 'object') {
        return (0, types_2.Failure)('Invalid parameters provided', {
    ...
    const { logger, timer } = (0, tool_context_helpers_1.setupToolContext)(context, 'verify-deploy');
    const { deploymentName: configDeploymentName, namespace: configNamespace, checks = ['pods', 'services', 'health'], } = params;
    const timeout = Math.floor(constants_1.DEFAULT_TIMEOUTS.verification / 1000); // Convert ms to seconds
    try {
        ...
        if (!configDeploymentName) {
            return (0, types_2.Failure)('Deployment name is required. Provide deploymentName parameter.', {
        ...
        const overallStatus = health.ready && (healthChecks.length === 0 || allHealthy)
            ? 'healthy'
            : health.ready
                ? 'unhealthy'
                : 'unknown';
        ...
        // Generate summary
        const summary = (0, summary_helpers_1.buildStatusSummary)(isSuccessful, `Deployment ${deploymentName} is healthy. ${health.readyReplicas}/${health.totalReplicas} pods ready. All health checks passing.`, `Deployment ${deploymentName} has issues. ${health.readyReplicas}/${health.totalReplicas} pods ready. Check pod logs for details.`);
        // Prepare the result
        const result = {
            summary,
        ...
        logger.info({ deploymentName, ready: health.ready, status: overallStatus }, 'Verification complete');
        timer.end({ deploymentName, ready: health.ready });
        return (0, types_2.Success)(result);
    }
    catch (error) {
        timer.error(error);
        return (0, types_2.Failure)((0, errors_1.extractErrorMessage)(error), {
...
exports.default = (0, tool_1.tool)({
    ...types_1.verifyDeployToolDefinition,
