...
const description = 'Gather insights from knowledgebase and return requirements for Kubernetes/Helm/ACA/Kustomize manifest creation. Supports repository analysis or ACA manifest conversion.';
const version = '2.0.0';
// Manifest type to topic mapping
const MANIFEST_TYPE_TO_TOPIC = {
    kubernetes: types_1.TOPICS.KUBERNETES,
    helm: types_1.TOPICS.GENERATE_HELM_CHARTS,
    aca: types_1.TOPICS.KUBERNETES,
    kustomize: types_1.TOPICS.KUBERNETES,
};
/**
 ...
 */
function parseAcaManifest(manifestStr) {
    try {
        // Try YAML first (most common for manifests)
        return js_yaml_1.default.load(manifestStr);
    }
    catch {
        try {
            // Fallback to JSON
 ...
 */
function analyzeAcaManifest(acaManifest) {
    const warnings = [];
    const containerApps = [];
    // Extract ACA properties
    const properties = (acaManifest.properties || acaManifest);
    ...
    const appName = acaManifest.name || 'aca-app';
    containerApps.push({
        name: appName,
 ...
 */
function planToManifestText(plan, manifestType) {
    const lines = [];
    ...
    if (manifestType === 'kubernetes') {
        ...
        const hasPrivileged = securityReqs.some((r) => r.recommendation.toLowerCase().includes('privileged'));
        if (hasPrivileged) {
        ...
        const hasHostNetwork = securityReqs.some((r) => r.recommendation.toLowerCase().includes('hostnetwork') ||
            r.recommendation.toLowerCase().includes('host network'));
        if (hasHostNetwork) {
            lines.push('      hostNetwork: true');
        }
        // Check for non-root user
        const hasNonRootUser = securityReqs.some((r) => r.recommendation.toLowerCase().includes('non-root') ||
        ...
        const hasResourceLimits = resourceReqs.some((r) => r.recommendation.toLowerCase().includes('resource') &&
 ...
 */
async function validatePlanAgainstPolicy(plan, manifestType, policyEvaluator, logger) {
    ...
    return (0, policy_helpers_1.validateContentAgainstPolicy)(manifestText, policyEvaluator, logger, 'manifest plan');
}
...
const runPattern = (0, knowledge_tool_pattern_1.createKnowledgeTool)({
    ...
    query: {
        topic: (input) => {
            ...
            if (input.acaManifest) {
                return types_1.TOPICS.CONVERT_ACA_TO_K8S;
            }
            return MANIFEST_TYPE_TO_TOPIC[input.manifestType];
        },
        category: types_2.CATEGORY.KUBERNETES,
        maxChars: 8000,
        maxSnippets: 20,
    ...
    categorization: {
        categoryNames: ['fieldMappings', 'security', 'resourceManagement', 'bestPractices'],
        categorize: (0, knowledge_tool_pattern_1.createSimpleCategorizer)({
            fieldMappings: (s) => Boolean(s.tags?.includes('mapping') ||
            ...
            resourceManagement: (s) => Boolean(s.tags?.includes('resources') ||
        ...
        buildPlan: (input, knowledge, _rules, confidence) => {
            ...
            if (input.acaManifest) {
                ...
                const bestPracticeMatches = (knowledge.categories.bestPractices || [])
                    .filter((snippet) => {
                    const isInMappings = (knowledge.categories.fieldMappings || []).some((s) => s.id === snippet.id);
                ...
                const summary = `🔨 ACTION REQUIRED: Convert ACA manifest to Kubernetes\n` +
                    `Container Apps: ${(0, summary_helpers_1.pluralize)(analysis.containerApps.length, 'app')} (${(0, summary_helpers_1.pluralize)(totalContainers, 'container')})\n` +
            ...
            const resourceMatches = (knowledge.categories.resourceManagement || []).map((snippet) => ({
                id: snippet.id,
                category: snippet.category || 'generic',
                recommendation: snippet.text,
                ...(snippet.tags && { tags: snippet.tags }),
            ...
            const bestPracticeMatches = (knowledge.categories.bestPractices || [])
                ...
                const isInSecurity = (knowledge.categories.security || []).some((s) => s.id === snippet.id);
                const isInResource = (knowledge.categories.resourceManagement || []).some((s) => s.id === snippet.id);
            ...
            const policyInstruction = input.k8sConfig
                ? ` Apply policy-driven configuration: ${input.k8sConfig.resourceDefaults
                    ? `resource limits (CPU: ${input.k8sConfig.resourceDefaults.cpuLimit}, Memory: ${input.k8sConfig.resourceDefaults.memoryLimit}), `
                    : ''}${input.k8sConfig.replicas ? `replicas: ${input.k8sConfig.replicas}, ` : ''}${input.k8sConfig.orgStandards?.namespace
            ...
            const frameworksStr = input.frameworks && input.frameworks.length > 0
                ? ` (${input.frameworks.map((f) => f.name).join(', ')})`
                : '';
            const policyConfigInfo = input.k8sConfig
                ? `Policy Config: ${input.k8sConfig.replicas || 1} replicas, ${input.k8sConfig.orgStandards?.namespace || 'default'} namespace\n`
                : '';
            const summary = `🔨 ACTION REQUIRED: Create ${input.manifestType} manifests\n` +
                `Application: ${input.name || input.language || 'application'}${frameworksStr}\n` +
                `Manifests: ${manifestFiles.map((f) => f.path.split('/').pop()).join(', ')}\n${policyConfigInfo}Recommendations: ${knowledgeMatches.length} total (${securityMatches.length} security, ${resourceMatches.length} resources, ${bestPracticeMatches.length} best practices)\n\n` +
...
async function handleGenerateK8sManifests(input, ctx) {
    const logger = (0, tool_helpers_1.getToolLogger)(ctx, name);
    ...
    if (input.acaManifest) {
        try {
            ...
            return (0, types_1.Failure)(`Invalid ACA manifest: ${(0, errors_1.extractErrorMessage)(error)}`, {
                message: `Invalid ACA manifest: ${(0, errors_1.extractErrorMessage)(error)}`,
    ...
    if (ctx.policy) {
        const policyValidation = await validatePlanAgainstPolicy(plan, input.manifestType, ctx.policy, logger);
        ...
        if (!policyValidation.passed) {
            const violationMessages = policyValidation.violations
                ...
                .join('\n');
            return (0, types_1.Failure)(`Generated manifest plan violates organizational policies:\n${violationMessages}`, {
        ...
        if (policyValidation.warnings.length > 0) {
            logger.warn({ warnings: policyValidation.warnings.map((w) => w.ruleId) }, 'Policy warnings in manifest plan');
        }
    }
    return result;
}
...
exports.default = (0, tool_1.tool)({
    ...
    category: 'kubernetes',
    version,
    schema: schema_1.generateK8sManifestsSchema,
    metadata: {
        knowledgeEnhanced: true,
    },
    chainHints: {
        success: 'Manifest plan generated successfully and passed policy validation. Next: Call prepare-cluster to create a kind cluster to deploy to.',
        failure: 'Manifest generation failed or plan violates policies. Review manifest requirements and policy violations.',
    },
