 ...
 * ```
 */
function validateAndEscapeClusterName(clusterName) {
    ...
    if (!nameRegex.test(clusterName)) {
        return (0, types_2.Failure)(`Invalid cluster name: "${clusterName}". Must contain only lowercase letters, numbers, and hyphens.`, {
    ...
    if (clusterName.length > 63) {
        return (0, types_2.Failure)(`Cluster name too long: "${clusterName}". Must be 63 characters or less.`, {
...
async function detectClusterPlatform(logger) {
    try {
        ...
        let os = 'linux';
        try {
            const { stdout: osOutput } = await execAsync("kubectl get nodes -o jsonpath='{.items[0].status.nodeInfo.operatingSystem}'");
        ...
        const platform = (0, platform_1.mapNodeArchToPlatform)(arch, os);
 ...
 */
async function validatePlatformCompatibility(targetPlatform, logger, warnings, strictMode) {
    ...
    if (!clusterPlatform) {
        ...
        if (strictMode) {
            return (0, types_2.Failure)(message, {
                message,
        ...
        return (0, types_2.Success)({
            clusterPlatform: null,
            compatible: false,
            requiresEmulation: false,
        });
    }
    const compatible = (0, platform_1.isPlatformCompatible)(targetPlatform, clusterPlatform);
    if (!compatible) {
        ...
        if (requiresEmulation) {
            ...
            const systemInfo = (0, platform_1.getSystemInfo)();
            const hostArch = process.arch;
            const isArmMacWithAmd64 = systemInfo.isMac &&
                hostArch === 'arm64' &&
                targetPlatform === 'linux/amd64' &&
                clusterPlatform === 'linux/amd64';
            if (isArmMacWithAmd64) {
                // This is expected for ARM Mac development targeting AMD64
                logger.info({ targetPlatform, clusterPlatform }, 'ARM Mac detected with AMD64 cluster - Docker emulation will be used');
                warnings.push(`Running AMD64 cluster on ARM Mac - images will use Docker emulation (may have performance impact)`);
                return (0, types_2.Success)({
                    clusterPlatform,
            ...
            logger.warn({ targetPlatform, clusterPlatform }, 'Target platform does not match cluster platform - may require emulation');
            warnings.push(`Platform mismatch: Building for ${targetPlatform} but cluster runs ${clusterPlatform}. Images may not run or may require emulation.`);
        }
        return (0, types_2.Success)({
            clusterPlatform,
            compatible: false,
...
async function checkConnectivity(k8sClient, logger) {
    try {
...
async function checkNamespace(k8sClient, namespace, logger) {
    try {
...
async function setupRbac(k8sClient, namespace, logger) {
    try {
...
async function checkIngressController(k8sClient, logger) {
    try {
...
async function checkKindInstalled(logger) {
    try {
...
async function installKind(logger) {
    try {
        logger.info('Installing kind...');
        const systemInfo = (0, platform_1.getSystemInfo)();
        const downloadOS = (0, platform_1.getDownloadOS)();
        const downloadArch = (0, platform_1.getDownloadArch)();
        let kindUrl;
        let kindExecutable;
        ...
        await (0, file_utils_1.downloadFile)(kindUrl, `./${kindExecutable}`);
        if (!systemInfo.isWindows) {
            await (0, file_utils_1.makeExecutable)(`./${kindExecutable}`);
        }
        if (systemInfo.isWindows) {
            try {
                ...
                try {
                    await execAsync(`mkdir "%USERPROFILE%\\bin" 2>nul & move ${kindExecutable} "%USERPROFILE%\\bin\\${kindExecutable}"`);
                }
                catch {
            ...
            try {
                ...
                try {
                    await execAsync(`mkdir -p ~/.local/bin && mv ./${kindExecutable} ~/.local/bin/${kindExecutable}`);
                }
                catch {
                    logger.warn('Failed to move kind executable to PATH, it may need manual installation');
                }
        ...
        throw new Error(`Kind installation failed: ${(0, errors_1.extractErrorMessage)(error)}`);
    }
}
/**
 ...
 */
async function checkKindClusterExists(clusterName, logger) {
    ...
    try {
        ...
        return (0, types_2.Success)(exists);
    }
    catch (error) {
        logger.debug({ error }, 'Error checking kind clusters');
        return (0, types_2.Success)(false);
    }
}
/**
 ...
 */
async function validateExistingClusterArchitecture(clusterName, targetPlatform, strictMode, logger) {
    if (!strictMode) {
        return (0, types_2.Success)(undefined);
    }
    logger.debug({ clusterName, targetPlatform }, 'Validating existing cluster architecture in strict mode');
    const clusterPlatform = await detectClusterPlatform(logger);
    if (!clusterPlatform) {
        return (0, types_2.Failure)('Could not detect existing cluster platform', {
    ...
    if (clusterPlatform !== targetPlatform) {
        return (0, types_2.Failure)(`Existing cluster '${clusterName}' is ${clusterPlatform} but target platform is ${targetPlatform}`, {
...
async function createKindCluster(clusterName, port, targetPlatform, strictMode, logger) {
    const escapedNameResult = validateAndEscapeClusterName(clusterName);
    if (!escapedNameResult.ok) {
        return escapedNameResult;
    }
    ...
    try {
        ...
        if (shouldUseAMD64Node) {
            logger.info({ hostArch, targetArch: 'amd64' }, 'Detected ARM Mac - creating AMD64 kind cluster for cross-platform compatibility (will use Docker emulation)');
        }
        else if (strictMode) {
            logger.info({ hostArch, targetPlatform, strictMode }, 'Strict mode enabled - creating cluster with native architecture');
        }
...
- |-
  [plugins."io.containerd.grpc.v1.cri".registry.mirrors."${constants_1.DOCKER.REGISTRY_HOST}:${port}"]
    endpoint = ["http://${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}:${constants_1.DOCKER.REGISTRY_INTERNAL_PORT}"]
  [plugins."io.containerd.grpc.v1.cri".registry.mirrors."${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}:${constants_1.DOCKER.REGISTRY_INTERNAL_PORT}"]
    endpoint = ["http://${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}:${constants_1.DOCKER.REGISTRY_INTERNAL_PORT}"]
nodes:
- role: control-plane
${nodeImageLine}
  ...
  extraPortMappings:
  - containerPort: ${constants_1.KUBERNETES.DEFAULT_HTTP_PORT}
    hostPort: ${constants_1.KUBERNETES.DEFAULT_HTTP_PORT}
    protocol: TCP
  - containerPort: ${constants_1.KUBERNETES.DEFAULT_HTTPS_PORT}
    hostPort: ${constants_1.KUBERNETES.DEFAULT_HTTPS_PORT}
    protocol: TCP
`;
        const configPath = await (0, file_utils_1.createTempFile)(kindConfig, '.yaml');
        try {
            ...
            await (0, file_utils_1.deleteTempFile)(configPath);
        }
    }
    catch (error) {
        logger.error({ clusterName, error }, 'Failed to create kind cluster');
        return (0, types_2.Failure)(`Kind cluster creation failed: ${(0, errors_1.extractErrorMessage)(error)}`);
    }
}
/**
 ...
 */
async function checkLocalRegistryExists(logger) {
    try {
        // Check if container exists (running or stopped)
        const { stdout: allContainers } = await execAsync(`docker ps -a --filter "name=${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}" --format "{{.Names}}"`);
        const containerExists = allContainers.trim() === constants_1.DOCKER.REGISTRY_CONTAINER_NAME;
        if (!containerExists) {
            logger.debug('Local registry container does not exist');
            return null;
        }
        // Get the port mapping for the existing container
        const { stdout: portMapping } = await execAsync(`docker inspect ${constants_1.DOCKER.REGISTRY_CONTAINER_NAME} --format '{{range $p, $conf := .NetworkSettings.Ports}}{{if eq $p "5000/tcp"}}{{(index $conf 0).HostPort}}{{end}}{{end}}'`);
        ...
        const { stdout: runningContainers } = await execAsync(`docker ps --filter "name=${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}" --format "{{.Names}}"`);
        const isRunning = runningContainers.trim() === constants_1.DOCKER.REGISTRY_CONTAINER_NAME;
        if (isRunning) {
        ...
        try {
            await execAsync(`docker start ${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}`);
            ...
            if (!healthCheck.healthy) {
                logger.warn({ attempts: healthCheck.attempts }, 'Registry health check failed after restart - may not be fully ready');
                // Continue anyway - health issues will be caught in later validation
            }
            else {
                logger.info({ attempts: healthCheck.attempts }, 'Registry health validated successfully after restart');
            }
            ...
            if (kindNetworkExists) {
                ...
                const isConnected = await checkContainerNetworkConnection(constants_1.DOCKER.REGISTRY_CONTAINER_NAME, 'kind', logger);
                if (!isConnected) {
                    logger.info('Registry not connected to kind network, reconnecting...');
                    try {
                        await execAsync(`docker network connect kind ${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}`);
                        logger.info('Registry reconnected to kind network successfully');
                        // Verify reconnection
                        const reconnected = await checkContainerNetworkConnection(constants_1.DOCKER.REGISTRY_CONTAINER_NAME, 'kind', logger);
                        if (!reconnected) {
                            logger.warn('Failed to verify registry reconnection to kind network');
                        }
                    }
                    catch (reconnectError) {
                        const errorMsg = (0, errors_1.extractErrorMessage)(reconnectError);
 ...
 */
async function validateRegistryHealth(port, logger) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            const { stdout: status } = await execAsync(`docker inspect ${constants_1.DOCKER.REGISTRY_CONTAINER_NAME} --format '{{.State.Status}}'`);
        ...
        try {
            const { stdout } = await execAsync(`curl -sf --max-time 3 http://${constants_1.DOCKER.REGISTRY_HOST}:${port}/v2/ || echo "failed"`, { timeout: 4000 });
    ...
    try {
        const { stdout: logs } = await execAsync(`docker logs --tail 20 ${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}`);
 ...
 */
async function checkDockerNetworkExists(networkName, logger) {
    try {
 ...
 */
async function checkContainerNetworkConnection(containerName, networkName, logger) {
    try {
        const { stdout } = await execAsync(`docker inspect ${containerName} --format '{{range $net, $v := .NetworkSettings.Networks}}{{$net}} {{end}}'`);
 ...
 */
async function waitForDockerNetwork(networkName, logger, maxAttempts = 10, delayMs = 1000) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
 ...
 */
async function getContainerNetworkIP(containerName, networkName, logger) {
    try {
        const { stdout } = await execAsync(`docker inspect ${containerName} --format '{{.NetworkSettings.Networks.${networkName}.IPAddress}}'`);
 ...
 */
async function verifyRegistryFromCluster(_port, logger) {
    try {
        ...
        const curlCommand = `curl -sf http://${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}:${constants_1.DOCKER.REGISTRY_INTERNAL_PORT}/v2/ && echo "success" || echo "failed"`;
        try {
            // Run test pod and wait for completion (timeout 30s)
            const { stdout } = await execAsync(`kubectl run ${testPodName} --image=curlimages/curl:latest --restart=Never --rm -i --timeout=30s -- sh -c '${curlCommand}'`, { timeout: 35000 });
 ...
 */
async function verifyRegistryDNSResolution(logger) {
    try {
        ...
        const nslookupCommand = `nslookup ${constants_1.DOCKER.REGISTRY_CONTAINER_NAME} && echo "DNS_SUCCESS" || echo "DNS_FAILED"`;
        try {
            // Run test pod and wait for completion (timeout 30s)
            const { stdout } = await execAsync(`kubectl run ${testPodName} --image=busybox:latest --restart=Never --rm -i --timeout=30s -- sh -c '${nslookupCommand}'`, { timeout: 35000 });
            ...
            logger.debug({ testPodName, resolves: success, resolvedIP, output: stdout.trim() }, 'In-cluster DNS resolution test result');
 ...
 */
async function validateContainerdConfig(clusterName, port, logger) {
    try {
        ...
        const { stdout } = await execAsync(`docker exec ${nodeContainerName} cat /etc/containerd/config.toml`);
        ...
        const mirrorHostPattern = new RegExp(`\\[plugins\\."io\\.containerd\\.grpc\\.v1\\.cri"\\.registry\\.mirrors\\."${constants_1.DOCKER.REGISTRY_HOST}:${port}"\\]`);
        ...
        const endpointPattern = new RegExp(`endpoint\\s*=\\s*\\["http://${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}:${constants_1.DOCKER.REGISTRY_INTERNAL_PORT}"\\]`);
        ...
        if (!isValid) {
            ...
            const registryConfigMatch = stdout.match(/\[plugins\."io\.containerd\.grpc\.v1\.cri"\.registry\.mirrors.*?\n(?:.*?\n){0,5}/);
 ...
 */
async function createLocalRegistryConfigMap(k8sClient, port, logger) {
    try {
        ...
        const configMap = {
            ...
            data: {
                'localRegistryHosting.v1': `host: "${constants_1.DOCKER.REGISTRY_HOST}:${port}"\nhelp: "https://kind.sigs.k8s.io/docs/user/local-registry/"`,
            },
        };
 ...
 */
async function createRegistryContainerOnly(port, logger) {
    try {
        logger.info({ port }, 'Creating local Docker registry container...');
        await execAsync(`docker run -d --restart=always -p ${port}:${constants_1.DOCKER.REGISTRY_INTERNAL_PORT} --name ${constants_1.DOCKER.REGISTRY_CONTAINER_NAME} registry:2`);
        ...
        throw new Error(`Registry container creation failed: ${(0, errors_1.extractErrorMessage)(error)}`);
    }
}
/**
 ...
 */
async function connectRegistryToKindNetwork(port, logger) {
    ...
    const alreadyConnected = await checkContainerNetworkConnection(constants_1.DOCKER.REGISTRY_CONTAINER_NAME, 'kind', logger);
    if (alreadyConnected) {
        logger.debug('Registry already connected to kind network');
    }
    else {
        ...
        try {
            ...
            const errorMsg = (0, errors_1.extractErrorMessage)(networkError);
        ...
        const connectedAfter = await checkContainerNetworkConnection(constants_1.DOCKER.REGISTRY_CONTAINER_NAME, 'kind', logger);
        if (!connectedAfter) {
        ...
        const registryIP = await getContainerNetworkIP(constants_1.DOCKER.REGISTRY_CONTAINER_NAME, 'kind', logger);
        if (registryIP) {
    ...
    const healthCheck = await validateRegistryHealth(port, logger);
    if (!healthCheck.healthy) {
        logger.warn({ attempts: healthCheck.attempts }, 'Registry health check failed - registry may not be fully ready');
    }
    else {
 ...
 */
async function setupKindCluster(clusterName, port, targetPlatform, strictMode, logger, checks) {
    ...
    if (!kindClusterExists) {
        ...
        await new Promise((resolve) => setTimeout(resolve, constants_1.DEFAULT_TIMEOUTS.clusterStabilization));
        ...
        const validationResult = await validateExistingClusterArchitecture(clusterName, targetPlatform, strictMode, logger);
 ...
 */
async function setupLocalRegistry(port, logger, checks) {
    ...
    if (existingPort !== null) {
        ...
        const registryUrl = `${constants_1.DOCKER.REGISTRY_HOST}:${existingPort}`;
    ...
    const registryUrl = `${constants_1.DOCKER.REGISTRY_HOST}:${port}`;
    const healthCheck = await validateRegistryHealth(port, logger);
 ...
 */
async function verifyClusterReadiness(k8sClient, namespace, shouldCreateNamespace, shouldSetupRbac, checkRequirements, installIngress, logger, checks, warnings) {
    // Check connectivity
    checks.connectivity = await checkConnectivity(k8sClient, logger);
    if (!checks.connectivity) {
        return (0, types_2.Failure)('Cannot connect to Kubernetes cluster', {
    ...
    if (!checks.permissions) {
        return (0, types_2.Failure)('Insufficient permissions for Kubernetes operations', {
    ...
    if (!checks.namespaceExists && shouldCreateNamespace) {
        ...
        if (ensureResult.ok) {
            ...
            return (0, types_2.Failure)(ensureResult.error || 'Failed to create namespace', ensureResult.guidance);
        }
    }
    ...
    return (0, types_2.Success)(clusterReady);
}
/**
 * Core cluster preparation implementation
 */
async function handlePrepareCluster(params, context) {
    const { logger, timer } = (0, tool_context_helpers_1.setupToolContext)(context, 'prepare-cluster');
    const { clusterType: explicitClusterType, environment = 'development', namespace = 'default', targetPlatform = 'linux/amd64', strictPlatformValidation = true, } = params;
    // Resolve effective cluster type: explicit clusterType wins, otherwise infer from environment for backwards compat
    const effectiveClusterType = explicitClusterType ?? (environment === 'development' ? 'kind' : 'generic');
    // Validate namespace
    const namespaceValidation = (0, validation_1.validateNamespace)(namespace);
    ...
    try {
        ...
        const checks = {
            ...
            rbacConfigured: undefined,
            kindInstalled: undefined,
            kindClusterCreated: undefined,
            localRegistryCreated: undefined,
            platformCompatible: undefined,
        };
        let localRegistryUrl;
        let localRegistryInfo;
        let registryPort;
        ...
        if (shouldSetupKind || shouldCreateLocalRegistry) {
            ...
            if (existingPort !== null) {
                ...
                registryPort = await (0, port_utils_1.findRegistryPort)();
        ...
        const k8sClient = (0, client_1.createKubernetesClient)(logger);
        ...
        if (shouldCreateLocalRegistry &&
            ...
            if (!containerdConfigValid) {
                warnings.push(`Containerd registry mirror configuration validation failed - image pulls from ${constants_1.DOCKER.REGISTRY_HOST}:${registryPort} may not work`);
            }
            else {
            ...
            if (!dnsResolution.resolves) {
                warnings.push(`Registry DNS resolution failed from within cluster - pods cannot resolve hostname '${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}'`);
            ...
            localRegistryInfo = {
                ...
                internalEndpoint: `${constants_1.DOCKER.REGISTRY_CONTAINER_NAME}:${constants_1.DOCKER.REGISTRY_INTERNAL_PORT}`,
                containerName: constants_1.DOCKER.REGISTRY_CONTAINER_NAME,
        ...
        const readinessResult = await verifyClusterReadiness(k8sClient, namespace, shouldCreateNamespace, shouldSetupRbac, checkRequirements, installIngress, logger, checks, warnings);
        ...
        const summary = `✅ Cluster prepared. Namespace '${namespace}' ${namespaceAction}. ${(0, summary_helpers_1.pluralize)(resourcesConfigured, 'resource')} configured. Ready for deployment.`;
        const result = {
            summary,
            success: true,
            clusterReady,
        ...
        return (0, types_2.Success)(result);
    }
    catch (error) {
        timer.error(error);
        const errorMessage = error instanceof Error ? error.message : String(error);
        return (0, types_2.Failure)(errorMessage, {
...
exports.prepareCluster = handlePrepareCluster;
const tool_1 = require("../../types/tool");
exports.default = (0, tool_1.tool)({
    ...types_1.prepareClusterToolDefinition,
