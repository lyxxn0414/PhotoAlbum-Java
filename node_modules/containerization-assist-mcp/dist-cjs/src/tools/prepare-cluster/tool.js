...
function validateAndEscapeClusterName(clusterName) {
    ...
    if (!nameRegex.test(clusterName)) {
        return (0, types_1.Failure)(`Invalid cluster name: "${clusterName}". Must contain only lowercase letters, numbers, and hyphens.`, {
    ...
    if (clusterName.length > 63) {
        return (0, types_1.Failure)(`Cluster name too long: "${clusterName}". Must be 63 characters or less.`, {
    ...
    return (0, types_1.Success)(`'${clusterName.replace(/'/g, "'\\''")}'`);
}
/**
...
async function validatePlatformCompatibility(targetPlatform, logger, warnings, strictMode) {
    ...
    if (!clusterPlatform) {
        ...
        if (strictMode) {
            return (0, types_1.Failure)(message, {
                message,
...
async function checkKindClusterExists(clusterName, logger) {
    ...
    try {
        ...
        return (0, types_1.Success)(exists);
    }
    catch (error) {
        logger.debug({ error }, 'Error checking kind clusters');
        return (0, types_1.Success)(false);
    }
}
/**
...
async function validateExistingClusterArchitecture(clusterName, targetPlatform, strictMode, logger) {
    ...
    const clusterPlatform = await detectClusterPlatform(logger);
    if (!clusterPlatform) {
        return (0, types_1.Failure)('Could not detect existing cluster platform', {
    ...
    if (clusterPlatform !== targetPlatform) {
        return (0, types_1.Failure)(`Existing cluster '${clusterName}' is ${clusterPlatform} but target platform is ${targetPlatform}`, {
...
`;
        ...
        return (0, types_1.Failure)(`Kind cluster creation failed: ${(0, errors_1.extractErrorMessage)(error)}`);
    }
}
/**
...
async function verifyClusterReadiness(k8sClient, namespace, shouldCreateNamespace, shouldSetupRbac, checkRequirements, installIngress, logger, checks, warnings) {
    ...
    if (!checks.connectivity) {
        return (0, types_1.Failure)('Cannot connect to Kubernetes cluster', {
    ...
    if (!checks.permissions) {
        return (0, types_1.Failure)('Insufficient permissions for Kubernetes operations', {
    ...
    if (!checks.namespaceExists && shouldCreateNamespace) {
        ...
        if (ensureResult.ok) {
            ...
            return (0, types_1.Failure)(ensureResult.error || 'Failed to create namespace', ensureResult.guidance);
        }
    }
    ...
    return (0, types_1.Success)(clusterReady);
}
/**
...
exports.default = (0, tool_1.tool)({
    ...
    category: 'kubernetes',
    version: '2.0.0',
    schema: schema_1.prepareClusterSchema,
    metadata: {
    ...
    chainHints: {
        ...
        failure: 'Cluster preparation found issues. Check connectivity, permissions, and namespace configuration.',
    },
