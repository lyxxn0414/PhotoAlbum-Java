...
const credential_helpers_1 = require("../../infra/docker/credential-helpers");
 ...
 * Push image handler
 */
async function handlePushImage(input, ctx) {
    const logger = (0, tool_helpers_1.getToolLogger)(ctx, 'push-image');
    const startTime = Date.now();
    try {
        // Validate required imageId
        if (!input.imageId) {
            return (0, types_1.Failure)('Missing required parameter: imageId', (0, errors_1.createErrorGuidance)('Missing required parameter: imageId', 'The imageId parameter is required to push an image', 'Provide the imageId of the Docker image to push. Use `docker images` to list available images.'));
        }
        // Parse and validate image name
        const parsedImage = (0, validation_helpers_1.parseImageName)(input.imageId);
        if (!parsedImage.ok) {
            return parsedImage;
        }
        ...
        const dockerClient = (ctx && 'docker' in ctx && ctx.docker) ||
            (0, client_1.createDockerClient)(logger);
        ...
        if (input.registry) {
            const credResult = await (0, credential_helpers_1.getRegistryCredentials)(input.registry, logger);
        ...
        if (!authConfig && input.credentials) {
            ...
            if (!input.credentials.username || !input.credentials.password) {
                return (0, types_1.Failure)('Missing registry credentials', (0, errors_1.createErrorGuidance)('Both username and password are required for registry authentication', 'Registry credentials are incomplete', 'Provide both username and password in the credentials parameter'));
            }
        ...
        if (!tagResult.ok) {
            return (0, types_1.Failure)(`Failed to tag image: ${tagResult.error}`, tagResult.guidance ||
                (0, errors_1.createErrorGuidance)(tagResult.error, 'Unable to tag the Docker image', 'Verify the image exists with `docker images` and the tag format is valid.'));
        }
        ...
        if (!pushResult.ok) {
            ...
            return (0, types_1.Failure)(`Failed to push image: ${pushResult.error}`, pushResult.guidance);
        }
        ...
        logger.info({ pushedTag, pushTime, digest: pushResult.value.digest }, 'Image pushed successfully');
        // Generate summary
        ...
        return (0, types_1.Failure)(`Push image failed: ${message}`, {
...
exports.default = (0, tool_1.tool)({
    ...types_2.pushImageToolDefinition,
