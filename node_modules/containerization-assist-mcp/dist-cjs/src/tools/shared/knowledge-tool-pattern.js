...
exports.defaultConfidenceCalculation = void 0;
exports.createKnowledgeTool = createKnowledgeTool;
exports.createSimpleCategorizer = createSimpleCategorizer;
const types_1 = require("../../types");
const matcher_1 = require("../../knowledge/matcher");
const policy_aware_matcher_1 = require("../../knowledge/policy-aware-matcher");
const loader_1 = require("../../knowledge/loader");
/**
 * Default confidence calculation.
 * Starts at 0.5 and increases by 0.05 per match, capped at 0.95.
 */
const defaultConfidenceCalculation = (matchCount) => {
    return matchCount > 0 ? Math.min(0.95, 0.5 + matchCount * 0.05) : 0.5;
};
exports.defaultConfidenceCalculation = defaultConfidenceCalculation;
/**
 ...
 * ```
 */
function createKnowledgeTool(config) {
    return async (input, ctx) => {
        // 1. Query knowledge base
        const filters = config.query.extractFilters(input);
        const topic = typeof config.query.topic === 'function' ? config.query.topic(input) : config.query.topic;
        ...
        if (ctx.policy) {
            ...
            const knowledgeData = await (0, loader_1.loadKnowledgeData)();
            ...
            const { snippets, filterResult } = await (0, policy_aware_matcher_1.getPolicyAwareKnowledgeSnippets)(knowledgeData.entries, query, ctx.policy);
            ...
            knowledgeSnippets = await (0, matcher_1.getKnowledgeSnippets)(topic, knowledgeOptions);
        }
        ...
        for (const snippet of knowledgeSnippets) {
            ...
            for (const category of categories) {
                ...
                if (categoryArray) {
                    ...
                    throw new Error(`categorize() returned undeclared category "${category}" for tool "${config.name}". ` +
        ...
        const calculateConfidence = config.confidence?.calculateConfidence || exports.defaultConfidenceCalculation;
        ...
        // Log completion
        const categoryCounts = Object.entries(categorized.categories).map(([name, snippets]) => `${name}: ${snippets.length}`);
        ctx.logger.info({
 ...
 * ```
 */
function createSimpleCategorizer(rules) {
    return (snippet) => {
        const categories = [];
        for (const [category, predicate] of Object.entries(rules)) {
...
//# sourceMappingURL=knowledge-tool-pattern.js.map