...
const types_2 = require("../../knowledge/types");
const knowledge_tool_pattern_1 = require("../shared/knowledge-tool-pattern");
const node_fs_1 = require("node:fs");
const node_path_1 = __importDefault(require("node:path"));
const policy_helpers_1 = require("../../lib/policy-helpers");
const types_3 = require("./types");
const { name } = types_3.generateDockerfileToolDefinition;
/**
 ...
 */
function extractBaseImages(lines) {
    return lines
 ...
 */
function hasHealthCheckInstruction(lines) {
    return lines.some((line) => line.toUpperCase().startsWith('HEALTHCHECK '));
}
/**
 * Check if Dockerfile has a non-root USER instruction
 */
function hasNonRootUserInstruction(lines) {
    return lines.some((line) => {
        const upper = line.toUpperCase();
        return (upper.startsWith('USER ') && !upper.startsWith('USER ROOT') && !upper.startsWith('USER 0'));
    });
}
/**
 * Count Dockerfile instructions
 */
function countInstructions(lines) {
    return lines.filter((line) => {
        const firstWord = line.split(/\s+/)[0];
        return (firstWord &&
            DOCKERFILE_KEYWORDS.includes(firstWord.toUpperCase()));
    }).length;
}
/**
 * Determine Dockerfile complexity based on instruction count and structure
 */
function determineComplexity(instructionCount, isMultistage) {
 ...
 */
function assessSecurityPosture(hasNonRootUser, hasHealthCheck) {
 ...
 */
function identifyPreservationNeeds(analysis) {
    const preserve = [];
 ...
 */
function identifyImprovementOpportunities(analysis, recommendations) {
    const improve = [];
    const addMissing = [];
 ...
 */
function determineEnhancementStrategy(analysis, preserve, improve, addMissing) {
 ...
 */
function generateEnhancementGuidance(analysis, recommendations) {
 ...
 */
function substituteImageVersion(image, targetVersion) {
    if (!targetVersion)
        return image;
 ...
 */
function createBaseImageRecommendation(snippet, languageVersion) {
    ...
    const size = sizeMatch?.[1] && sizeMatch[2] ? `${sizeMatch[1]}${sizeMatch[2].toUpperCase()}` : undefined;
    return {
        image,
        category,
        reason: snippet.text,
...
const runPattern = (0, knowledge_tool_pattern_1.createKnowledgeTool)({
    name,
    query: {
        topic: types_1.TOPICS.DOCKERFILE,
        category: types_2.CATEGORY.DOCKERFILE,
        maxChars: 8000,
        maxSnippets: 20,
    ...
    categorization: {
        categoryNames: ['baseImages', 'security', 'optimization', 'bestPractices'],
        categorize: (0, knowledge_tool_pattern_1.createSimpleCategorizer)({
            baseImages: (s) => Boolean(s.tags?.includes('base-image') ||
            ...
            optimization: (s) => Boolean(s.tags?.includes('optimization') ||
    ...
    rules: {
        applyRules: (input) => {
            ...
            const shouldUseMultistage = language === 'java' ||
                language === 'go' ||
                language === 'rust' ||
                language === 'dotnet' ||
                language === 'c#' ||
    ...
    plan: {
        buildPlan: (input, knowledge, rules, confidence) => {
            ...
            let baseImageMatches = (knowledge.categories.baseImages || [])
            ...
            const securityMatches = (knowledge.categories.security || [])
                .map((snippet) => ({
                id: snippet.id,
            ...
            const optimizationMatches = (knowledge.categories.optimization || [])
                .map((snippet) => ({
                id: snippet.id,
            ...
            const bestPracticeMatches = (knowledge.categories.bestPractices || [])
                ...
                const isInSecurity = (knowledge.categories.security || []).some((s) => s.id === snippet.id);
                const isInOptimization = (knowledge.categories.optimization || []).some((s) => s.id === snippet.id);
            ...
            const dockerfilePath = existingDockerfile
                ...
                : node_path_1.default.join(modulePath, 'Dockerfile');
            const relativeDockerfilePath = node_path_1.default.relative(path, dockerfilePath) || './Dockerfile';
            // Build nextAction directive
            const nextAction = existingDockerfile
                ? {
                    action: 'update-files',
            ...
            const totalRecommendations = baseImageMatches.length +
 ...
 */
function planToDockerfileText(plan) {
    const lines = [];
    ...
    const hasNonRootUser = security.some((s) => s.recommendation.toLowerCase().includes('non-root user') ||
        s.recommendation.toLowerCase().includes('user directive'));
    const hasHealthCheck = security.some((s) => s.recommendation.toLowerCase().includes('healthcheck'));
 ...
 */
async function isImageCompliant(image, evaluator, logger) {
    ...
    const validation = await (0, policy_helpers_1.validateContentAgainstPolicy)(dockerfileText, evaluator, logger, 'base image');
    ...
    const imageSpecificViolations = validation.violations.filter((v) => {
    ...
    if (!isCompliant) {
        logger.debug({
            image,
            violations: imageSpecificViolations.map((v) => v.ruleId),
 ...
 */
async function validatePlanAgainstPolicy(plan, evaluator, logger) {
    ...
    return (0, policy_helpers_1.validateContentAgainstPolicy)(dockerfileText, evaluator, logger, 'Dockerfile plan');
}
async function handleGenerateDockerfile(input, ctx) {
    const path = input.repositoryPath || '';
    if (!path) {
        return (0, types_1.Failure)('Path is required. Provide a path parameter.', {
    ...
    const pathResult = await (0, validation_helpers_1.validatePathOrFail)(path, {
        mustExist: true,
    ...
    const dockerfilePath = node_path_1.default.join(targetPath, 'Dockerfile');
    ...
    try {
        ...
        ctx.logger.info({ error, path: dockerfilePath }, 'No existing Dockerfile found - will create new one');
    }
    // Query policy for generation configuration (if policy is available)
    let dockerfileConfig = null;
    if (ctx.policy) {
        const configQuery = await ctx.queryConfig('containerization.generation_config', {
            language: input.language || 'auto-detect',
            framework: input.framework,
            environment: input.environment || 'production',
    ...
    if (ctx.policy) {
        ...
        const templateQuery = await ctx.queryConfig('containerization.templates.templates', {
            language: input.language || 'auto-detect',
            framework: input.framework,
            environment: input.environment || 'production',
            appName: input.repositoryPath?.split('/').pop() || 'app',
        ...
        if (templateQuery) {
            ...
            const { mergeTemplatesIntoPlan } = await Promise.resolve().then(() => __importStar(require('../../lib/template-merger')));
        ...
        const dynamicDefaultsQuery = await ctx.queryConfig('containerization.dynamic_defaults.defaults', {
            language: input.language || 'auto-detect',
            environment: input.environment || 'production',
    ...
    if (ctx.policy && plan.recommendations.baseImages.length > 0) {
        ctx.logger.info({ count: plan.recommendations.baseImages.length, hasPolicyConfig: !!ctx.policy }, 'Filtering base images against policy');
        ...
        for (const imageRec of plan.recommendations.baseImages) {
            ...
            if (isCompliant) {
                ...
                ctx.logger.info({ image: imageRec.image }, 'Base image filtered out due to policy violation');
            }
        }
        ...
        if (filteredCount > 0) {
            ctx.logger.info({ filtered: filteredCount, remaining: filteredBaseImages.length }, 'Base images filtered by policy');
        }
    }
    else if (!ctx.policy) {
    ...
    if (ctx.policy && (plan.recommendations.securityConsiderations.length > 0 ||
         ...
         */
        const extractImageReferences = (text) => {
            const images = [];
         ...
         */
        const containsCompliantImagesOnly = async (entry) => {
    ...
    if (ctx.policy) {
        ...
        if (!policyValidation.passed) {
            ctx.logger.warn({
                violations: policyValidation.violations.map((v) => ({
                    rule: v.ruleId,
                    message: v.message,
                })),
        ...
        if (policyValidation.warnings.length > 0) {
            ctx.logger.warn({ warnings: policyValidation.warnings.map((w) => w.ruleId) }, 'Policy warnings in Dockerfile plan');
        }
    }
    return result;
}
...
exports.default = (0, tool_1.tool)({
    ...types_3.generateDockerfileToolDefinition,
