...
exports.extractK8sErrorGuidance = extractK8sErrorGuidance;
...
const k8sErrorPatterns = [
    ...
    (0, error_guidance_1.customPattern)((error) => {
        ...
        resolution: 'Verify cluster is running: `kubectl cluster-info`. Check API server address in kubeconfig and ensure network connectivity.',
    }),
    // Timeout issues
    (0, error_guidance_1.customPattern)((error) => {
        const msg = getErrorMessage(error).toLowerCase();
        ...
        resolution: 'Check cluster connectivity and load. Verify firewall rules allow access to the API server. Try `kubectl get nodes` to test connectivity.',
    }),
    ...
    (0, error_guidance_1.customPattern)((error) => {
        ...
        resolution: 'Refresh cluster credentials. For cloud providers: re-authenticate (e.g., `aws eks update-kubeconfig`, `gcloud container clusters get-credentials`).',
    }),
    ...
    (0, error_guidance_1.customPattern)((error) => {
        ...
        resolution: 'Verify resource name and namespace. Use `kubectl get <resource> -n <namespace>` to list available resources.',
    }),
    // Namespace issues
    (0, error_guidance_1.customPattern)((error) => {
        const msg = getErrorMessage(error).toLowerCase();
        ...
        resolution: 'Create the namespace: `kubectl create namespace <name>` or ensure it exists before deploying resources.',
    }),
    // Resource conflicts
    (0, error_guidance_1.customPattern)((error) => {
        const msg = getErrorMessage(error).toLowerCase();
        ...
        resolution: 'Use a different name, delete the existing resource, or use `kubectl apply` instead of `create` to update it.',
    }),
    // Validation errors
    (0, error_guidance_1.customPattern)((error) => {
        const msg = getErrorMessage(error).toLowerCase();
        ...
        resolution: 'Check the manifest against Kubernetes API documentation. Use `kubectl apply --dry-run=client` to validate syntax.',
    }),
    ...
    (0, error_guidance_1.customPattern)((error) => {
        ...
        resolution: 'Check cluster version with `kubectl version` and update API versions in manifests. Some resources may require cluster upgrades.',
    }),
];
/**
 * Default guidance when no pattern matches
 */
function defaultK8sGuidance(error) {
    const message = getErrorMessage(error);
    return {
        message: message || 'Kubernetes operation failed',
        hint: 'An error occurred during the Kubernetes operation',
        resolution: 'Run `kubectl get events --sort-by=.lastTimestamp` to see recent cluster events. Check resource logs with `kubectl logs`.',
 ...
 */
function extractK8sErrorGuidance(error, operation) {
    const baseExtractor = (0, error_guidance_1.createErrorGuidanceBuilder)(k8sErrorPatterns, defaultK8sGuidance);
