...
exports.createKubernetesClient = void 0;
const k8s = __importStar(require("@kubernetes/client-node"));
const types_1 = require("../../types");
const errors_1 = require("./errors");
const kubeconfig_discovery_1 = require("./kubeconfig-discovery");
const resource_operations_1 = require("./resource-operations");
 ...
 */
const createKubernetesClient = (logger, kubeconfig, timeout) => {
    ...
    if (kubeconfig) {
        ...
        const validation = (0, kubeconfig_discovery_1.discoverAndValidateKubeconfig)();
        if (!validation.ok) {
    ...
    const fetchDeploymentStatus = async (namespace, name) => {
        try {
            ...
            const guidance = (0, errors_1.extractK8sErrorGuidance)(error, 'get deployment status');
    ...
    return {
         ...
         */
        async applyManifest(manifest, namespace = 'default') {
            // Set namespace in metadata if not already set and not a cluster-scoped resource
            const isClusterScoped = ['Namespace', 'ClusterRole', 'ClusterRoleBinding'].includes(manifest.kind || '');
            ...
            const result = await (0, resource_operations_1.applyResource)(kc, workingManifest, logger);
            if (!result.ok) {
        ...
        async ping() {
            ...
            try {
                ...
                const guidance = (0, errors_1.extractK8sErrorGuidance)(error, 'ping cluster');
                logger.debug({
         ...
         */
        async checkPermissions(namespace) {
            try {
        ...
        async checkIngressController() {
            try {
                ...
                if (ingressClasses.items.length > 0) {
                    const hasDefault = ingressClasses.items.some((ic) => ic.metadata?.annotations?.['ingressclass.kubernetes.io/is-default-class'] === 'true');
                ...
                const hasIngress = deployments.items.some((d) => d.metadata?.name?.includes('ingress') ||
        ...
        async ensureNamespace(namespace) {
            try {
                ...
                const guidance = (0, errors_1.extractK8sErrorGuidance)(error, 'ensure namespace');
         ...
         */
        async waitForDeploymentReady(namespace, name, timeoutSeconds, pollIntervalMs) {
            try {
                const pollInterval = pollIntervalMs ?? DEPLOYMENT_POLL_INTERVAL_MS;
                const startTime = Date.now();
                const maxWaitTime = timeoutSeconds * 1000;
                logger.debug({ namespace, name, timeoutSeconds, pollInterval }, 'Waiting for deployment to be ready');
                ...
                const guidance = (0, errors_1.extractK8sErrorGuidance)(error, 'wait for deployment ready');
...
exports.createKubernetesClient = createKubernetesClient;
