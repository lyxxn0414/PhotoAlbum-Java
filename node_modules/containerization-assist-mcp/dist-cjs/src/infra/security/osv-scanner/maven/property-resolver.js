...
exports.extractLocalProperties = extractLocalProperties;
exports.extractLocalDependencyManagement = extractLocalDependencyManagement;
exports.resolvePropertyValue = resolvePropertyValue;
...
function flattenProperties(obj, prefix = '') {
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        if (typeof value === 'string') {
            ...
            const nested = flattenProperties(value, fullKey);
...
function extractLocalProperties(project, logger) {
    ...
    if (project.parent?.groupId && project.parent?.artifactId && project.parent?.version) {
        ...
        logger.debug({ parent: parentKey }, 'Parent POM detected but remote fetching is disabled - using local properties only');
    }
    return properties;
}
/** Extract local dependency management from current POM only (no parent fetching) */
function extractLocalDependencyManagement(project) {
    const depMgmt = new Map();
...
function resolvePropertyValue(value, properties, project) {
    let resolved = value;
