 ...
 */
async function readDockerConfig(logger) {
    try {
        const configPath = (0, path_1.join)((0, os_1.homedir)(), '.docker', 'config.json');
        ...
        return (0, types_1.Success)(config);
    }
    catch (error) {
        ...
        return (0, types_1.Failure)(`Failed to read Docker config: ${errorMessage}`, {
...
function normalizeRegistryHostname(registry) {
    let hostname;
    try {
 ...
 */
function validateCredentialHelperResponse(expectedRegistry, credentialResponse, logger) {
 ...
 */
async function executeCredentialHelper(helperName, serverUrl, logger) {
    return new Promise((resolve) => {
        const helperCommand = `docker-credential-${helperName}`;
        logger.debug({ helperCommand, serverUrl }, 'Executing credential helper');
        const child = (0, child_process_1.spawn)(helperCommand, ['get'], {
        ...
        child.on('error', (error) => {
            ...
            if (errorMessage.includes('ENOENT') || errorMessage.includes('command not found')) {
                resolve((0, types_1.Failure)(`Credential helper not found: ${helperCommand}`, {
                ...
                resolve((0, types_1.Failure)(`Credential helper failed: ${errorMessage}`, {
        ...
        child.on('close', (code) => {
            if (code !== 0) {
                ...
                if (errorMessage.includes('credentials not found') || errorMessage.includes('not logged in')) {
                    resolve((0, types_1.Failure)('No credentials found for registry', {
            ...
            try {
                ...
                if (!result.Username || !result.Secret || !result.ServerURL) {
                    resolve((0, types_1.Failure)('Invalid credential helper response', {
                ...
                resolve((0, types_1.Success)(result));
            }
            catch (parseError) {
                const errorMessage = parseError instanceof Error ? parseError.message : String(parseError);
                resolve((0, types_1.Failure)(`Failed to parse credential helper response: ${errorMessage}`, {
 ...
 */
async function getRegistryCredentials(registry, logger) {
    try {
        // SECURITY: Validate input to prevent malicious registry hostnames
        if (!registry || typeof registry !== 'string') {
            return (0, types_1.Failure)('Invalid registry hostname', {
        ...
        return (0, types_1.Success)(null);
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        return (0, types_1.Failure)(`Failed to get registry credentials: ${errorMessage}`, {
...
//# sourceMappingURL=credential-helpers.js.map