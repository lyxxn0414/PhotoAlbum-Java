 ...
 */
async function fetchDockerHubMetadata(imageName, tag, logger) {
    try {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    ...
                    logger.warn({ imageName, tag, retryAfter }, 'Docker Hub rate limit exceeded. Please try again later.');
                }
                else {
        ...
        if (error instanceof Error && error.name === 'AbortError') {
            logger.warn({ imageName, tag, timeoutMs: DOCKER_HUB_REQUEST_TIMEOUT_MS }, `Docker Hub request timed out after ${DOCKER_HUB_REQUEST_TIMEOUT_MS / 1000} seconds`);
        }
        else {
 ...
 */
function getEstimatedImageSize(imageName, tag) {
 ...
 */
async function getImageMetadata(imageName, tag, logger) {
 ...
 */
function getRegistryApiUrl(registryUrl) {
 ...
 */
async function listDockerHubTags(repository, logger) {
    try {
        ...
        try {
            ...
            clearTimeout(timeoutId);
            if (!response.ok) {
                return (0, types_1.Failure)(`Docker Hub returned status ${response.status}`, {
        ...
        return (0, types_1.Failure)(`Failed to list Docker Hub tags: ${errorMessage}`, {
 ...
 */
async function listPrivateRegistryTags(repository, logger, authConfig) {
    try {
        ...
        try {
            ...
            if (authConfig) {
                const authString = Buffer.from(`${authConfig.username}:${authConfig.password}`).toString('base64');
            ...
            if (!response.ok) {
                if (response.status === 401) {
                    return (0, types_1.Failure)('Authentication required', {
                ...
                return (0, types_1.Failure)(`Registry returned status ${response.status}`, {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        return (0, types_1.Failure)(`Failed to list private registry tags: ${errorMessage}`, {
 ...
 */
async function authenticateRegistry(docker, config, logger) {
    try {
        ...
        if (config.username && config.password) {
            ...
            return (0, types_1.Failure)('Authentication requires either username/password or token', {
        ...
        try {
            ...
            return (0, types_1.Success)(authConfig);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            return (0, types_1.Failure)(`Authentication failed: ${errorMessage}`, {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        return (0, types_1.Failure)(`Authentication error: ${errorMessage}`, {
 ...
 */
async function checkRegistryHealth(registryUrl, logger) {
    try {
        ...
        try {
            ...
            logger.debug({ registryUrl: apiUrl, status: response.status, healthy: isHealthy }, 'Registry health check complete');
            return (0, types_1.Success)(isHealthy);
        }
        catch (error) {
            clearTimeout(timeoutId);
            if (error instanceof Error && error.name === 'AbortError') {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        return (0, types_1.Failure)(`Health check error: ${errorMessage}`, {
 ...
 */
async function checkImageExists(docker, imageName, logger) {
    try {
        ...
        try {
            ...
            return (0, types_1.Failure)(`Failed to check image existence: ${errorMessage}`, {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        return (0, types_1.Failure)(`Image existence check error: ${errorMessage}`, {
 ...
 */
async function listRepositoryTags(repository, logger, authConfig) {
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        return (0, types_1.Failure)(`Failed to list tags: ${errorMessage}`, {
