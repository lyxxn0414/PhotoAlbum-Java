...
exports.extractDockerErrorGuidance = extractDockerErrorGuidance;
const error_guidance_1 = require("../../lib/error-guidance");
/**
 * Type guard to check if an error has dockerode-specific properties
 */
function hasDockerodeProperties(error) {
    const dockerError = error;
    return (typeof dockerError.statusCode === 'number' ||
 ...
 */
function extractDockerMessage(error) {
...
const dockerErrorPatterns = [
    ...
    (0, error_guidance_1.customPattern)((error) => {
        ...
        resolution: 'Check your internet connection and verify the registry URL is correct. For private registries, ensure DNS is configured properly.',
    ...
    (0, error_guidance_1.customPattern)((error) => {
        ...
        resolution: 'Check network connectivity, firewall rules, or try again. For large images, consider increasing timeout settings.',
        details: buildDetails(error),
    })),
    (0, error_guidance_1.customPattern)((error) => {
        if (!(error instanceof Error))
        ...
        resolution: 'Retry the operation. If it persists, check Docker daemon health and network stability.',
        details: buildDetails(error),
    })),
    (0, error_guidance_1.customPattern)((error) => {
        if (!(error instanceof Error))
        ...
        resolution: 'Retry the operation. If it persists, check your DNS configuration and network connectivity.',
        details: buildDetails(error),
    })),
    (0, error_guidance_1.customPattern)((error) => {
        if (!(error instanceof Error))
        ...
        resolution: 'Check firewall rules, VPN connection, and network configuration. Verify the registry is accessible from your network.',
        details: buildDetails(error),
    })),
    (0, error_guidance_1.customPattern)((error) => {
        if (!(error instanceof Error))
    ...
    (0, error_guidance_1.customPattern)((error) => {
        ...
        resolution: 'Verify you have push/pull permissions for this repository. Contact registry administrator if needed.',
        details: buildDetails(error),
    })),
    (0, error_guidance_1.customPattern)((error) => {
        if (!(error instanceof Error))
        ...
        resolution: 'Verify the image name and tag are correct. Use `docker images` to list local images or check registry catalog.',
        details: buildDetails(error),
    })),
    ...
    (0, error_guidance_1.customPattern)((error) => {
        ...
        const detailedMessage = hasDockerodeProperties(dockerError)
            ...
            : dockerError.message || 'Registry server error';
        return {
            ...
            resolution: 'The registry is experiencing problems. Retry after a moment or check registry status page.',
            details: buildDetails(dockerError),
        };
    }),
    ...
    (0, error_guidance_1.messagePattern)('connect ENOENT', {
        message: 'Docker daemon is not running',
        hint: 'Cannot connect to Docker socket',
        resolution: 'Start Docker daemon: `sudo systemctl start docker` (Linux) or start Docker Desktop (Mac/Windows).',
    }),
    // Dockerfile syntax errors
    (0, error_guidance_1.messagePattern)('unknown instruction', {
        message: 'Invalid Dockerfile instruction detected',
        hint: 'Unknown or misspelled Dockerfile instruction',
        resolution: 'Check your Dockerfile for syntax errors. Valid instructions include FROM, RUN, COPY, ADD, CMD, ENTRYPOINT, etc.',
    }),
    (0, error_guidance_1.customPattern)((error) => {
        ...
        resolution: 'Ensure your Dockerfile starts with a FROM instruction and follows proper syntax.',
    })),
    // Missing Dockerfile
    (0, error_guidance_1.customPattern)((error) => {
        const msg = error instanceof Error ? error.message : String(error);
 ...
 */
function defaultDockerGuidance(error) {
    if (error instanceof Error) {
        return {
 ...
 */
function extractDockerErrorGuidance(error) {
    const baseExtractor = (0, error_guidance_1.createErrorGuidanceBuilder)(dockerErrorPatterns, defaultDockerGuidance);
