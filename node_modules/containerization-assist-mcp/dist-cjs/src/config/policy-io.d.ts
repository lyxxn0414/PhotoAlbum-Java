import { type Result } from '../types';
import { type RegoEvaluator } from './policy-rego';
/**
 * Load Rego policy from file
 *
 * @param file - Path to .rego policy file
 * @returns Promise<Result<RegoEvaluator>>
 */
export declare function loadPolicy(file: string): Promise<Result<RegoEvaluator>>;
/**
 * Load and merge multiple Rego policy files
 *
 * @param policyPaths - Array of .rego policy file paths
 * @returns Promise<Result<RegoEvaluator>>
 */
export declare function loadAndMergePolicies(policyPaths: string[]): Promise<Result<RegoEvaluator>>;
/**
 * Clear the policy cache
 * Useful for testing or when policies are updated
 */
export declare function clearPolicyCache(): void;
//# sourceMappingURL=policy-io.d.ts.map