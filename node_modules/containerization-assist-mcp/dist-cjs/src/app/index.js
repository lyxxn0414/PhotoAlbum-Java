...
const checks_1 = require("../infra/health/checks");
const chain_hints_1 = require("./chain-hints");
const loader_1 = require("../knowledge/loader");
/**
 ...
 */
function applyToolAliases(tools, aliases) {
    if (!aliases) {
...
function createApp(config = {}) {
    const logger = config.logger || (0, logger_1.createLogger)({ name: 'containerization-assist' });
    // Initialize tool logging file at startup
    (0, tool_logger_1.createToolLoggerFile)(logger);
    const tools = config.tools || tools_1.ALL_TOOLS;
    ...
    const registryTools = aliasedTools.map((tool) => tool);
    const toolsMap = new Map();
    for (const tool of registryTools) {
        toolsMap.set(tool.name, tool);
    }
    const chainHintsMode = config.chainHintsMode || orchestrator_types_1.CHAINHINTSMODE.ENABLED;
    const outputFormat = config.outputFormat || mcp_server_1.OUTPUTFORMAT.NATURAL_LANGUAGE;
    const orchestratorConfig = {
        chainHintsMode,
        chainHints: chain_hints_1.DEFAULT_CHAIN_HINTS,
        aliasToOriginalMap,
    };
    ...
    function buildOrchestrator() {
        return (0, orchestrator_1.createOrchestrator)({
            registry: toolsMap,
            logger,
    ...
    const orchestratedExecute = (request) => {
    ...
    return {
         ...
         */
        execute: async (toolName, params, metadata) => orchestratedExecute({
            toolName: toolName,
            params,
            metadata: {
        ...
        startServer: async (transport) => {
            ...
            try {
                (0, loader_1.loadKnowledgeBase)();
            }
            catch (error) {
                logger.error({ error }, 'Failed to load knowledge base during server startup');
                throw new Error(`Server startup failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            ensureOrchestrator();
            const serverOptions = {
                logger,
            ...
            const mcpServer = (0, mcp_server_1.createMCPServer)(toolList, serverOptions, orchestratedExecute);
        ...
        healthCheck: async () => {
            ...
            const [dockerStatus, k8sStatus] = await Promise.all([
                (0, checks_1.checkDockerHealth)(logger),
                (0, checks_1.checkKubernetesHealth)(logger),
            ]);
            const hasIssues = !dockerStatus.available || !k8sStatus.available;
            const status = hasIssues ? 'unhealthy' : 'healthy';
            return {
                status,
                tools: toolCount,
                message: hasIssues
         ...
         */
        getLogFilePath: () => {
            return (0, tool_logger_1.getLogFilePath)();
        },
    };
}
