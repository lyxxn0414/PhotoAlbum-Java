...
exports.extractSafeTelemetryMetrics = extractSafeTelemetryMetrics;
exports.createSafeTelemetryEvent = createSafeTelemetryEvent;
...
function hashValue(value) {
    return (0, node_crypto_1.createHash)('sha256').update(value).digest('hex').substring(0, 16);
}
/**
 ...
 */
function extractSafeTelemetryMetrics(toolName, result) {
    const metrics = {};
    ...
    switch (toolName) {
        case 'analyze-repo':
            ...
            if ('framework' in result)
                metrics.framework = result.framework;
            if ('language' in result)
                metrics.language = result.language;
            if ('moduleCount' in result)
                metrics.moduleCount = result.moduleCount;
            break;
        case 'build-image':
            // Safe: Build duration, image size (aggregates)
            if ('buildTime' in result)
                metrics.buildTimeMs = result.buildTime;
            if ('size' in result)
                metrics.imageSizeBytes = result.size;
        ...
        case 'verify-deploy':
            ...
            if ('replicas' in result)
                metrics.replicas = result.replicas;
            if ('readyReplicas' in result)
                metrics.readyReplicas = result.readyReplicas;
            if ('status' in result)
 ...
 */
function createSafeTelemetryEvent(toolName, params, result, durationMs) {
    const event = {
        toolName,
        success: result.ok,
