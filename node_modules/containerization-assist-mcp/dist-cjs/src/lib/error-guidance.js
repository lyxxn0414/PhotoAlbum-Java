...
exports.createErrorGuidanceBuilder = createErrorGuidanceBuilder;
 ...
 * ```
 */
function createErrorGuidanceBuilder(patterns, defaultGuidance) {
    return function extractGuidance(error) {
 ...
 * ```
 */
function messagePattern(substring, guidance) {
    return {
        match: (error) => {
            const message = (0, errors_1.extractErrorMessage)(error).toLowerCase();
 ...
 * ```
 */
function customPattern(matchFn, guidance) {
    return {
        match: matchFn,
