...
exports.findRegistryPort = findRegistryPort;
 ...
 */
async function findRegistryPort() {
    const startPort = config_1.DOCKER.REGISTRY_PORT_START;
    const endPort = config_1.DOCKER.REGISTRY_PORT_END;
    const port = await findAvailablePort(startPort, endPort);
    if (port === null) {
        throw new Error(`No available ports found in range ${startPort}-${endPort}. Please free up some ports and try again.`);
    }
    return port;
}
