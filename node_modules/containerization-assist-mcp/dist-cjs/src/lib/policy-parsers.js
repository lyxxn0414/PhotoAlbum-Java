...
exports.extractPorts = extractPorts;
exports.calculateResourceRatio = calculateResourceRatio;
exports.extractManifestImages = extractManifestImages;
exports.extractServicePorts = extractServicePorts;
...
function parseCPU(cpu) {
    ...
    if (trimmed.endsWith('m')) {
        ...
        if (isNaN(value) || value < 0) {
            return (0, types_1.Failure)('Invalid CPU value', {
                message: `Cannot parse CPU value: ${cpu}`,
                hint: 'Expected format: "1000m", "1.5", or "2"',
            });
        }
        return (0, types_1.Success)(value);
    }
    ...
    return (0, types_1.Success)(value * 1000);
}
/**
...
function parseMemory(memory) {
    ...
    for (const [unit, multiplier] of Object.entries(units)) {
        if (trimmed.endsWith(unit)) {
            ...
            if (isNaN(value) || value < 0) {
                return (0, types_1.Failure)('Invalid memory value', {
                    message: `Cannot parse memory value: ${memory}`,
                    hint: 'Expected format: "512Mi", "2Gi", "1G", etc.',
                });
            }
            return (0, types_1.Success)(value * multiplier);
        }
    }
 ...
 * // "node:18-alpine"
 */
function extractImageName(dockerfile) {
 ...
 * // [3000, 8080]
 */
function extractPorts(dockerfile) {
    const lines = dockerfile.split('\n');
    const ports = [];
    for (const line of lines) {
        const trimmed = line.trim();
    ...
    return (0, types_1.Success)(ports);
}
/**
 ...
 */
function calculateResourceRatio(limit, request) {
    if (request <= 0) {
        return (0, types_1.Failure)('Invalid request value', {
            message: 'Resource request must be greater than 0',
            hint: 'Check your resource configuration',
        });
    }
    if (limit < request) {
        return (0, types_1.Failure)('Invalid limit value', {
            message: 'Resource limit cannot be less than request',
            hint: 'Limit should be >= request',
        });
    }
    return (0, types_1.Success)(limit / request);
}
/**
 ...
 * // ["myapp:v1"]
 */
function extractManifestImages(manifest) {
    if (typeof manifest !== 'object' || manifest === null) {
        return (0, types_1.Failure)('Invalid manifest', {
            message: 'Manifest must be an object',
        });
    }
    const images = [];
    // Helper to extract images from container specs
    const extractFromContainers = (containers) => {
    ...
    if ('spec' in manifest &&
        ...
        if ('template' in spec &&
            ...
            typeof spec.template.spec === 'object') {
    ...
    return (0, types_1.Success)(images);
}
/**
 ...
 * // [3000]
 */
function extractServicePorts(service) {
    if (typeof service !== 'object' || service === null) {
        return (0, types_1.Failure)('Invalid service', {
    ...
    if (!('ports' in spec) || !Array.isArray(spec.ports)) {
        return (0, types_1.Success)([]); // No ports defined
    }
    const targetPorts = [];
    ...
    return (0, types_1.Success)(targetPorts);
}
//# sourceMappingURL=policy-parsers.js.map