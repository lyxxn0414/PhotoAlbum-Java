...
- **Source JDK path (Java 8)**: C:\Users\yueli6\.jdk\jdk-8\bin
- **Target JDK path (Java 17)**: C:\Program Files\Microsoft\jdk-17.0.16.8-hotspot\bin
...
## Upgrade Goals
- Upgrade Java to 17 <!-- java:*:17 -->
- Upgrade Spring Boot to 3.4.x <!-- org.springframework.boot:*:3.4.x -->

## Guidelines
> Note: You can add any specific guidelines or constraints for the upgrade process here if needed, bullet points are preferred.
- Oracle-specific native SQL queries in PhotoRepository must remain functional\n- Update Maven compiler settings from Java 8 to Java 17\n- Migrate all javax.* namespace imports (javax.persistence, javax.validation, javax.imageio) to Jakarta EE (jakarta.*) where applicable\n- Keep javax.imageio as-is since it is part of Java SE, not Jakarta EE\n- Update Dockerfile to use JDK 17 base images\n- Update Hibernate dialect references if needed for Spring Boot 3.x compatibility\n- Update ojdbc8 to ojdbc17 for Java 17 compatibility
...
3. Achieve the objectives set for each milestone one by one until the final upgrade goal is reached. At this stage, focus only on upgrading the required dependencies and Java version to meet the upgrade targets and ensuring the project can be built.
    - Milestone 1: Upgrade project to use `Spring Boot 3.3.x`, `Java 17`
    - Milestone 2: Upgrade project to use `Spring Boot 3.4.x`
